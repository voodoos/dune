      ...
      let parse =
        record
          (let%map loc = loc
           ...
           and generate_runner =
             field_o "generate_runner" (located Action.Unexpanded.decode)
           and extends =
             field "extends" (list (located Lib_name.decode)) ~default:[]
           in
...
include Sub_system.Register_end_point (struct
        ...
        ~else_:
          (record
             (let%map loc = loc
              ...
              in
              {loc; deps; flags; backend; libraries}))
  end
  ...
  let gen_rules c ~(info : Info.t) ~backends =
      ...
      Module.Name.Map.singleton main_module_name
        (Module.make main_module_name
           ~impl:
             ...
             ; syntax = OCaml }
           ~visibility:Public ~obj_name:name)
    in
    let bindings =
    ...
    let runner_libs =
      ...
      >>= fun more_libs -> Lib.closure ~linking:true (lib @ libs @ more_libs)
    in
       ...
       >>> Build.all
             (List.filter_map backends ~f:(fun (backend : Backend.t) ->
                  Option.map backend.info.generate_runner
                    ~f:(fun (loc, action) ->
                      SC.Action.run sctx action ~loc ~bindings ~dir
                        ~dep_kind:Required ~targets:Alias ~targets_dir:dir
                        ~scope ) ))
       >>^ (fun actions -> Action.with_stdout_to target (Action.progn actions))
       >>> Build.action_dyn ~targets:[target] ());
    let cctx =
      Compilation_context.create () ~super_context:sctx ~scope
        ~dir:inline_test_dir ~modules ~opaque:false ~requires:runner_libs
    ...
    let flags =
      ...
      Build.all
        (List.map flags ~f:(fun flags ->
             Super_context.expand_and_eval_set sctx flags ~scope ~dir ~bindings
               ~standard:(Build.return []) ))
      >>^ List.concat
    in
    SC.add_alias_action sctx ~loc:(Some info.loc)
        ...
        (A.progn
           ...
           :: ( Module.Name.Map.values source_modules
              |> List.concat_map ~f:(fun m ->
                     [Module.file m Impl; Module.file m Intf] )
