...
module Gen (P : Install_rules.Params) = struct
  ...
  let build_lib (lib : Library.t) ~scope ~flags ~dir ~obj_dir ~mode
    ...
    Option.iter (Context.compiler ctx mode) ~f:(fun compiler ->
        ...
        let map_cclibs =
          ...
          if ctx.ccomp_type = "msvc" then msvc_hack_cclibs else fun x -> x
        in
        ...
        let obj_deps =
          ...
          | Byte -> obj_deps
          | Native ->
              obj_deps >>> Build.paths (artifacts modules ~ext:ctx.ext_obj)
        in
  ...
  let build_alias_module
      {Lib_modules.Alias_module.main_module_name; alias_module} ~modules
      ~modules_of_vlib ~cctx ~dynlink ~js_of_ocaml =
    let file =
    ...
    Module_compilation.build_module cctx alias_module ~js_of_ocaml ~dynlink
      ~sandbox:alias_module_build_sandbox
      ~dep_graphs:(Ocamldep.Dep_graphs.dummy alias_module)

  let build_wrapped_compat_modules (lib : Library.t) cctx ~js_of_ocaml ~dynlink
  ...
  let build_cxx_file (lib : Library.t) ~scope ~dir ~includes (loc, src, dst) =
    ...
    let output_param =
      if ctx.ccomp_type = "msvc" then [Concat ("", [A "/Fo"; Target dst])]
      else [A "-o"; Target dst]
    in
    SC.add_rule sctx ~loc
  ...
  let ocamlmklib (lib : Library.t) ~dir ~scope ~o_files ~sandbox ~custom
      ~targets =
    SC.add_rule sctx ~sandbox ~loc:lib.buildable.loc
      ( SC.expand_and_eval_set sctx ~scope ~dir lib.c_library_flags
          ~standard:(Build.return [])
      >>> Build.run ~dir:ctx.build_dir (Ok ctx.ocamlmklib)
            [ A "-g"
  ...
  let build_self_stubs lib ~scope ~dir ~o_files =
    ...
    let modes =
      Mode_conf.Set.eval lib.modes ~has_native:(Option.is_some ctx.ocamlopt)
    in
    ...
    let resolve_name ~ext (loc, fn) =
      ...
      if
        not
          ...
          | None -> false
          | Some p -> Path.Set.mem all_dirs p )
      then
        Errors.fail loc
          "File %a is not part of the current directory group. This is not \
           allowed."
          Path.pp
      ...
      Arg_spec.S
        ...
        ; Arg_spec.of_result_map requires ~f:(fun libs ->
              S
                ...
                ; Hidden_deps
                    (Lib_file_deps.L.file_deps sctx libs ~exts:[".h"]) ] ) ]
    in
  ...
  let build_shared lib ~dir ~flags ~(ctx : Context.t) =
    Option.iter ctx.ocamlopt ~f:(fun ocamlopt ->
        let src =
          Library.archive lib ~dir ~ext:(Mode.compiled_lib_ext Native)
        in
        let dst = Library.archive lib ~dir ~ext:".cmxs" in
        let build =
          Build.dyn_paths
            (Build.arr (fun () -> [Library.archive lib ~dir ~ext:ctx.ext_lib]))
          >>> Ocaml_flags.get flags Native
          >>> Build.run ~dir:ctx.build_dir (Ok ocamlopt)
  ...
  let setup_file_deps lib ~dir ~obj_dir ~modules ~modules_of_vlib =
    let add_cms ~cm_kind ~init =
      Module.Name.Map.fold ~init ~f:(fun m acc ->
    ...
    List.iter Cm_kind.all ~f:(fun cm_kind ->
        ...
        Lib_file_deps.setup_file_deps_alias sctx ~dir lib
          ~exts:[Cm_kind.ext cm_kind] files );
    Lib_file_deps.setup_file_deps_group_alias sctx ~dir lib
      ~exts:[".cmi"; ".cmx"];
  ...
  let setup_build_archives (lib : Dune_file.Library.t) ~wrapped_compat ~cctx
      ~(dep_graphs : Ocamldep.Dep_graphs.t) ~vlib_dep_graphs =
    ...
    let modules =
      List.rev_append
        ...
        (Module.Name_map.impl_only modules_of_vilb)
    in
    ...
    let top_sorted_modules =
      ...
      | Some (vlib_dep_graphs : Ocamldep.Dep_graphs.t) ->
          Ocamldep.Dep_graph.top_closed_multi_implementations
            [vlib_dep_graphs.impl; dep_graphs.impl]
            modules
    in
  ...
  let library_rules (lib : Library.t) ~dir_contents ~dir ~scope ~compile_info
    ...
    let pp =
      Preprocessing.make sctx ~dir ~dep_kind ~scope
        ...
        ~preprocessor_deps:
          (SC.Deps.interpret sctx ~scope ~dir lib.buildable.preprocessor_deps)
      ...
      Compilation_context.create () ~super_context:sctx
        ?modules_of_vlib:
          (Option.map impl ~f:(fun impl ->
               ...
               |> Lib_modules.modules ))
        ~scope ~dir ~dir_kind ~obj_dir ~private_obj_dir ~modules ?alias_module
        ?lib_interface_module:(Lib_modules.lib_interface_module lib_modules)
        ~flags ~requires ~preprocessing:pp ~no_keep_locs:lib.no_keep_locs
        ~opaque ?stdlib:lib.stdlib
    in
    let dynlink =
      Dynlink_supported.get lib.dynlink ctx.supports_shared_libraries
    in
    ...
    build_wrapped_compat_modules lib cctx ~dynlink ~js_of_ocaml ~modules
      ~wrapped_compat;
    let vlib_dep_graphs, dep_graphs =
      let dep_graphs = Ocamldep.rules cctx in
      match impl with
      | None -> (None, dep_graphs)
      | Some impl ->
          let vlib = Virtual_rules.Implementation.vlib_dep_graph impl in
          (Some vlib, Ocamldep.Dep_graphs.merge_for_impl ~vlib ~impl:dep_graphs)
    in
    Module_compilation.build_modules cctx ~js_of_ocaml ~dynlink ~dep_graphs;
    let vlib_modules =
      Option.map ~f:Virtual_rules.Implementation.vlib_modules impl
    in
    if Option.is_none lib.stdlib then
      Option.iter
        (Lib_modules.alias lib_modules)
        ~f:
          (build_alias_module ~modules:source_modules ~cctx ~dynlink
             ~js_of_ocaml
  ...
  let rules (lib : Library.t) ~dir_contents ~dir ~scope ~dir_kind :
    ...
    SC.Libs.with_lib_deps sctx compile_info ~dir ~f:(fun () ->
        library_rules lib ~dir_contents ~dir ~scope ~compile_info ~dir_kind )
end
