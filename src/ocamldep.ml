...
module Dep_graph = struct
  ...
  let top_closed t modules =
    ...
    match
      Module.Name.Top_closure.top_closure modules ~key:Module.name
        ~deps:(fun m ->
          Module.name m |> Module.Name.Map.find per_module |> Option.value_exn
      )
    with
    | Ok modules -> modules
    | Error cycle ->
        die "dependency cycle between modules in %s:\n   %a"
          (Path.to_string t.dir) pp_cycle cycle
  ...
  module Multi = struct
    let top_closed_multi (ts : t list) modules =
      ...
      >>^ fun per_module ->
      let per_obj =
        Module.Obj_map.of_list_reduce per_module ~f:List.rev_append
      in
  ...
  let make_top_closed_implementations ~name ~f ts modules =
    Build.memoize name
      (let filter_out_intf_only = List.filter ~f:Module.has_impl in
       f ts (filter_out_intf_only modules) >>^ filter_out_intf_only)

  let top_closed_multi_implementations =
    make_top_closed_implementations ~name:"top sorted multi implementations"
      ~f:Multi.top_closed_multi

  let top_closed_implementations =
    make_top_closed_implementations ~name:"top sorted implementations"
      ~f:top_closed
  ...
  let wrapped_compat ~modules ~wrapped_compat =
    { dir = Path.root
    ; per_module =
        Module.Name.Map.merge wrapped_compat modules ~f:(fun _ d m ->
            match (d, m) with
...
module Dep_graphs = struct
  ...
  let dummy m = Ml_kind.Dict.make_both (Dep_graph.dummy m)
  ...
  let merge_impl ~(ml_kind : Ml_kind.t) _ vlib impl =
    ...
    | Some (mv, _), Some (mi, i) ->
        if
          Module.obj_name mv = Module.obj_name mi
          && Module.intf_only mv && Module.impl_only mi
        then match ml_kind with Impl -> Some (mi, i) | Intf -> None
        else if Module.is_private mv || Module.is_private mi then Some (mi, i)
        else
        ...
        { impl with
          per_module =
            Module.Name.Map.merge ~f:(merge_impl ~ml_kind)
              (Ml_kind.Dict.get vlib ml_kind).per_module impl.per_module } )
end
...
let parse_module_names ~(unit : Module.t) ~modules ~modules_of_vlib words =
  ...
  List.filter_map words ~f:(fun m ->
      ...
      if m = Module.name unit then None
      else
...
let parse_deps cctx ~file ~unit lines =
  ...
  | [line] -> (
    ...
    | Some (basename, deps) -> (
        ...
        if Option.is_none stdlib then
          Option.iter lib_interface_module ~f:(fun (m : Module.t) ->
              ...
              if
                ...
                && List.exists deps ~f:(fun x -> Module.name x = m)
              then
                die
                  "Module %a in directory %s depends on %a.\n\
                   This doesn't make sense to me.\n\n\
                   %a is the main module of the library and is the only \
                   module exposed \n\
                   outside of the library. Consequently, it should be the one \
                   depending \n\
        ...
        match stdlib with
        | None -> (
          match alias_module with None -> deps | Some m -> m :: deps )
        | Some {modules_before_stdlib; _} -> (
            if Module.Name.Set.mem modules_before_stdlib (Module.name unit)
            then deps
            else
              match CC.lib_interface_module cctx with
              | None -> deps
              | Some m ->
                  if Module.name unit = Module.name m then deps else m :: deps
            ) ) )

let deps_of cctx ~ml_kind unit =
  let sctx = CC.super_context cctx in
  if is_alias_module cctx unit then Build.return []
  else
    ...
    | Some file ->
        ...
        let build_paths dependencies =
          let dependency_file_path m =
            let file_path m =
              if is_alias_module cctx m then None
              else
        ...
        Build.memoize
          ...
          >>^ parse_module_names ~modules_of_vlib ~unit
                ~modules:(CC.modules cctx) )
...
let rules_generic cctx ~modules =
  Ml_kind.Dict.of_func (fun ~ml_kind ->
      ...
      in
      {Dep_graph.dir = CC.dir cctx; per_module} )
...
let graph_of_remote_lib ~obj_dir ~modules =
  ...
  Ml_kind.Dict.of_func (fun ~ml_kind ->
      let per_module =
        Module.Name.Map.map modules ~f:(fun m -> (m, deps_of ~ml_kind m))
      in
      {Dep_graph.dir = obj_dir; per_module} )
