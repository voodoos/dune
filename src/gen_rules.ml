...
module For_stanza = struct
  ...
  let empty_none = {merlin = None; cctx = None; js = None}

  let empty_list = {merlin = []; cctx = []; js = []}
  ...
  let rev t = {t with merlin = List.rev t.merlin; cctx = List.rev t.cctx}
end
...
module Gen (P : Install_rules.Params) = struct
  ...
  let gen_rules dir_contents cctxs
    ...
    let for_stanza ~dir = function
      | Library lib ->
          let cctx, merlin =
            Lib_rules.rules lib ~dir ~scope ~dir_contents ~dir_kind
          in
      ...
      | Executables exes ->
          let cctx, merlin =
            Exe_rules.rules exes ~sctx ~dir ~scope ~dir_contents ~dir_kind
          in
          { For_stanza.merlin = Some merlin
      ...
      | Copy_files {glob; _} ->
          let source_dirs =
            ...
            |> Path.parent_exn |> Path.Set.singleton
          in
          { For_stanza.merlin = Some (Merlin.make ~source_dirs ())
          ; cctx = None
          ; js = None }
    ...
    in
    let {For_stanza.merlin = merlins; cctx = cctxs; js = js_targets} =
    ...
    Super_context.add_alias_deps sctx
      ~dyn_deps:
        (Build.paths_matching ~dir:ctx_dir ~loc:Loc.none (fun p ->
  ...
  let gen_rules ~dir components : Build_system.extra_sub_directories_to_keep =
    ...
    | _ -> (
      ...
      | Some _ -> (
          ...
          | Standalone -> ignore (gen_rules dir_contents [] ~dir : _ list)
  ...
  let init () =
    let module Install_rules = Install_rules.Gen (P) in
    Install_rules.init ();
    Lib_rules.Odoc.init ()
end
...
module type Gen = sig
  val gen_rules :
    dir:Path.t -> string list -> Build_system.extra_sub_directories_to_keep
...
let gen ~contexts ~build_system ?(external_lib_deps_mode = false)
    ?only_packages conf =
  let open Fiber.O in
  ...
  let make_sctx (context : Context.t) : _ Fiber.t =
    ...
    in
    let stanzas () =
      Dune_load.Dune_files.eval ~context dune_files
      >>| fun stanzas ->
      match only_packages with
      | None -> stanzas
      | Some pkgs ->
          List.map stanzas ~f:(fun (dir_conf : Dune_load.Dune_file.t) ->
              {dir_conf with stanzas = relevant_stanzas pkgs dir_conf.stanzas}
          )
    in
    Fiber.fork_and_join host stanzas
    >>= fun (host, stanzas) ->
    let sctx =
      Super_context.create ?host ~build_system ~context ~projects ~file_tree
        ~packages ~external_lib_deps_mode ~stanzas
    in
    let module M = Gen (struct
      let sctx = sctx
    end) in
    Fiber.Ivar.fill (Option.value_exn (Hashtbl.find sctxs context.name)) sctx
    >>| fun () -> (context.name, (module M : Gen))
  in
  Fiber.parallel_map contexts ~f:make_sctx
  >>| fun l ->
