...
module DB :
  ...
  *)
    ...
    val create_from_findlib : ?external_lib_deps_mode:bool -> Findlib.t -> t

    val find :
      t -> Lib_name.t -> (lib, Error.Library_not_available.Reason.t) result

    val find_many : t -> loc:Loc.t -> Lib_name.t list -> lib list Or_exn.t
...
module Sub_system :
  sig
    ...
    val dump_config :
      lib -> (Syntax.Version.t * Dune_lang.t) Sub_system_name.Map.t
  end
  with type lib := t
