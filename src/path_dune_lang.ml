...
let encode p =
  ...
  in
  let make constr =
    Dune_lang.List [Dune_lang.atom constr; Dune_lang.atom_or_quoted_string arg]
  in
  if is_in_build_dir p then make "In_build_dir"
  else if is_in_source_tree p then make "In_source_tree"
  else make "External"
...
let decode =
  ...
  let external_ =
    plain_string (fun ~loc t ->
        ...
        else Path.of_string ~error_loc:loc t )
  in
  sum
