...
let setup ?(log = Log.no_log) ?external_lib_deps_mode ?workspace
    ?workspace_file ?only_packages ?extra_ignored_subtrees ?x
    ?ignore_promoted_rules ?(capture_outputs = true) ?profile () =
  ...
  Option.iter only_packages ~f:(fun set ->
      Package.Name.Set.iter set ~f:(fun pkg ->
          if not (Package.Name.Map.mem conf.packages pkg) then
            ...
            die
              "@{<error>Error@}: I don't know about package %s (passed \
               through --only-packages/--release)%s"
              pkg_name
              (hint pkg_name
  ...
  List.iter contexts ~f:(fun (ctx : Context.t) ->
      Log.infof log "@[<1>Dune context:@,%a@]@." Sexp.pp (Context.to_sexp ctx)
  );
  let rule_done = ref 0 in
  let rule_total = ref 0 in
  let gen_status_line () =
    { Scheduler.message = Some (sprintf "Done: %u/%u" !rule_done !rule_total)
    ; show_jobs = true }
  in
  ...
  in
  Gen_rules.gen conf ~build_system ~contexts ?only_packages
...
let find_context_exn t ~name =
  ...
  | Some ctx -> ctx
  | None -> die "@{<Error>Error@}: Context %S not found!@." name
...
let external_lib_deps ?log ~packages () =
  Scheduler.go ?log
    ...
    Path.Map.map
      ...
      ~f:
        (Lib_name.Map.filteri ~f:(fun name _ ->
             not (Lib_name.Set.mem internals name) )) )

let ignored_during_bootstrap =
  Path.Set.of_list (List.map ~f:Path.in_source ["test"; "example"])
...
let set_concurrency ?log (config : Config.t) =
  ...
  >>= fun n -> if n >= 1 then Scheduler.set_concurrency n else Fiber.return ()
...
let bootstrap () =
  ...
  let main () =
    ...
    let config =
      ...
      if !profile <> Some "release" then Config.load_user_config_file ()
      else Config.default
    in
    let config =
      Config.merge config {display = !display; concurrency = !concurrency}
    in
    let config =
    ...
    Scheduler.go ~log ~config
      ...
      setup ~log
        ...
        ?profile:!profile ~extra_ignored_subtrees:ignored_during_bootstrap ()
