...
module DB = struct
  ...
  let create ~projects ~context ~installed_libs ~ext_lib ~ext_obj internal_libs
      =
    let projects_by_name =
      ...
      | Error (_name, project1, project2) ->
          ...
          Exn.code_error "Scope.DB.create got two projects with the same name"
            [("project1", to_sexp project1); ("project2", to_sexp project2)]
    in
    ...
    let public_libs =
      ...
      in
      Lib.DB.create () ~parent:installed_libs
        ~resolve:(fun name ->
          ...
          | Some project ->
              let scope =
                Option.value_exn
                  (Project_name_map.find !by_name_cell
    ...
    let by_name =
      ...
      Project_name_map.merge projects_by_name libs_by_project_name
        ~f:(fun _name project libs ->
          ...
          in
          let root =
            Path.append_local build_context_dir (Dune_project.root project)
          in
