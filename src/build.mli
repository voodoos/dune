...
val action : ?dir:Path.t -> targets:Path.t list -> Action.t -> (_, Action.t) t

val action_dyn :
  ?dir:Path.t -> targets:Path.t list -> unit -> (Action.t, Action.t) t
...
val copy_and_add_line_directive :
  src:Path.t -> dst:Path.t -> (unit, Action.t) t
