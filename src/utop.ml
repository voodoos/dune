...
let libs_under_dir sctx ~db ~dir =
  ...
  >>= (fun dir ->
        ...
        >>| File_tree.Dir.fold ~traverse_ignored_dirs:true ~init:[]
              ~f:(fun dir acc ->
                let dir =
                  Path.append
                ...
                | Some (d : Super_context.Dir_with_dune.t) ->
                    List.fold_left d.stanzas ~init:acc ~f:(fun acc -> function
                      | Dune_file.Library l -> (
                        match
                          Lib.DB.find_even_when_hidden db (Library.best_name l)
                        with
                        | None ->
...
let setup sctx ~dir =
  ...
  in
  let cctx =
    Compilation_context.create () ~super_context:sctx ~scope ~dir:utop_exe_dir
      ~modules ~opaque:false ~requires
      ~flags:
