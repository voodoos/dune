...
struct
  ...
  let vlib_stubs_o_files {Implementation.vlib; _} = Lib.foreign_objects vlib

  let setup_copy_rules_for_impl ~dir {Implementation.vlib; impl; vlib_modules}
      =
    ...
    let target_obj_dir =
      ...
      fun m -> if Module.is_public m then obj_dir else private_obj_dir
    in
    let copy_to_obj_dir ~obj_dir file =
      let dst = Path.relative obj_dir (Path.basename file) in
      Super_context.add_rule ~loc:(Loc.of_pos __POS__) sctx
    ...
    let copy_obj_file m ext =
      ...
      copy_to_obj_dir ~obj_dir:(target_obj_dir m) source
    in
    let copy_module m =
      ...
      if Module.has_impl m then (
        if modes.byte then copy_obj_file m (Cm_kind.ext Cmo);
  ...
  let check_module_fields ~(lib : Dune_file.Library.t) ~virtual_modules
    ...
    let new_public_modules =
      Module.Name.Map.foldi modules ~init:[] ~f:(fun name m acc ->
          if
            Module.is_public m
            && not (Module.Name.Map.mem virtual_modules name)
          then name :: acc
          else acc )
    in
    if new_public_modules <> [] then
    ...
    let missing_modules, impl_modules_with_intf, private_virtual_modules =
      Module.Name.Map.foldi virtual_modules ~init:([], [], [])
        ~f:(fun m _ (mms, ims, pvms) ->
          ...
          | Some m ->
              let ims =
                if Module.has_intf m then Module.name m :: ims else ims
              in
              let pvms =
                if Module.is_public m then pvms else Module.name m :: pvms
              in
              (mms, ims, pvms) )
    in
    if private_virtual_modules <> [] then
      (* The loc here will never be none as we've some private modules *)
      Errors.fail_opt
        (Option.bind lib.private_modules ~f:Ordered_set_lang.loc)
        "These private modules cannot be private:\n%s"
        (module_list private_virtual_modules);
    if missing_modules <> [] then
      Errors.fail lib.buildable.loc
        "Library %a cannot implement %a because the following modules lack an \
         implementation:\n\
         %s"
        Lib_name.Local.pp (snd lib.name) Lib_name.pp implements
        (module_list missing_modules);
    if impl_modules_with_intf <> [] then
      Errors.fail lib.buildable.loc
  ...
  let impl ~(lib : Dune_file.Library.t) ~scope ~modules =
    Option.map lib.implements ~f:(fun (loc, implements) ->
        ...
        | Error _ ->
            Errors.fail loc
              "Cannot implement %a as that library isn't available" Lib_name.pp
              implements
        | Ok vlib ->
            let virtual_modules =
              Option.map (Lib.virtual_ vlib)
                ~f:(fun (v : Lib_info.Virtual.t) -> v.modules )
            in
            let vlib_modules =
              ...
              | Some Unexpanded ->
                  let dir_contents =
                    Dir_contents.get sctx ~dir:(Lib.src_dir vlib)
                  in
            ...
            {Implementation.impl = lib; vlib; vlib_modules} )
end
