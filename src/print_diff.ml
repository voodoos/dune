...
let print ?(skip_trailing_cr = Sys.win32) path1 path2 =
  let dir, file1, file2 =
    ...
    | _ -> (Path.root, Path.to_string path1, Path.to_string path2)
  in
  ...
  | Some cmd ->
      ...
      die "command reported no differences: %s"
        ...
        else sprintf "cd %s && %s" (quote_for_shell (Path.to_string dir)) cmd
        )
  | None -> (
      if Config.inside_dune then fallback ()
        ...
        | Some prog ->
            Process.run ~dir ~env:Env.initial Strict prog
              ...
              ; "-location-style"
              ; "omake"
              ; ( if Lazy.force Colors.stderr_supports_colors then "-unrefined"
                else "-ascii" )
              ; file1
              ; file2 ]
            >>= fun () ->
