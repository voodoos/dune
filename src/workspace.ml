...
let env_field =
  field_o "env"
    (Syntax.since syntax (1, 1) >>= fun () -> Dune_env.Stanza.decode)
...
module Context = struct
  module Target = struct
    ...
    let add ts x =
      match x with
      | None -> ts
      | Some t -> if List.mem t ~set:ts then ts else ts @ [t]
  end
  ...
  module Name = struct
    let t =
      plain_string (fun ~loc name ->
          ...
          then
            of_sexp_errorf loc "%S is not allowed as a build context name" name;
          name )
  end
  ...
  module Common = struct
    ...
    let t ~profile =
      ...
      and loc = loc in
      {targets; profile; loc; env; toolchain}
  end
  ...
  module Default = struct
    ...
    let t ~profile ~x =
      Common.t ~profile >>| fun t -> {t with targets = Target.add t.targets x}
  end
  ...
  let name = function Default _ -> "default" | Opam o -> o.name
  ...
  let default ?x ?profile () =
    Default
      ...
      ; profile = Option.value profile ~default:Config.default_build_profile
      ; env = None
      ; toolchain = None }
end
...
let t ?x ?profile:cmdline_profile () =
  ...
  let merlin_context =
    List.fold_left contexts ~init:None ~f:(fun acc ctx ->
        ...
        if String.Set.mem !defined_names name then
          Errors.fail (Context.loc ctx) "second definition of build context %S"
            name;
        defined_names :=
  ...
  in
  {merlin_context; contexts = List.rev contexts; env}
