...
let get_std_output : _ -> Process.std_output_to = function
  | None -> Terminal
  | Some (fn, oc) ->
      Opened_file {filename = fn; tail = false; desc = Channel oc}
...
let exec_run_direct ~ectx ~dir ~env ~stdout_to ~stderr_to prog args =
  ...
  | Some ({Context.for_host = Some host; _} as target) ->
      let invalid_prefix prefix =
        ...
        | Some _ ->
            die
              "Context %s has a host %s.@.It's not possible to execute binary \
               %a in it.@.@.This is a bug and should be reported upstream."
              target.name host.name Path.pp prog
      in
      invalid_prefix (Path.relative Path.build_dir target.name);
      invalid_prefix (Path.relative Path.build_dir ("install/" ^ target.name))
  );
  Process.run Strict ~dir ~env ~stdout_to ~stderr_to ~purpose:ectx.purpose prog
    args
...
let rec exec t ~ectx ~dir ~env ~stdout_to ~stderr_to =
  ...
  | Chdir (dir, t) -> exec t ~ectx ~dir ~env ~stdout_to ~stderr_to
  | Setenv (var, value, t) ->
      exec t ~ectx ~dir ~stdout_to ~stderr_to ~env:(Env.add env ~var ~value)
  ...
  | Progn l -> exec_list l ~ectx ~dir ~env ~stdout_to ~stderr_to
  | Echo strs -> exec_echo stdout_to (String.concat strs ~sep:" ")
  | Cat fn ->
      Io.with_file_in fn ~f:(fun ic ->
          let oc =
            match stdout_to with None -> stdout | Some (_, oc) -> oc
          in
  ...
  | Copy (src, dst) ->
      ...
      Fiber.return ()
  | Symlink (src, dst) ->
      ( if Sys.win32 then Io.copy_file ~src ~dst ()
      else
        let src =
  ...
  | Digest_files paths ->
      let s =
        ...
        in
        Digest.string (Marshal.to_string data [])
      in
...
and redirect outputs fn t ~ectx ~dir ~env ~stdout_to ~stderr_to =
  ...
  let stdout_to, stderr_to =
    ...
    | Outputs -> (out, out)
  in
  exec t ~ectx ~dir ~env ~stdout_to ~stderr_to >>| fun () -> close_out oc

and exec_list l ~ectx ~dir ~env ~stdout_to ~stderr_to =
  match l with
  | [] -> Fiber.return ()
  | [t] -> exec t ~ectx ~dir ~env ~stdout_to ~stderr_to
  | t :: rest ->
      exec t ~ectx ~dir ~env ~stdout_to ~stderr_to
      >>= fun () -> exec_list rest ~ectx ~dir ~env ~stdout_to ~stderr_to
