...
let of_library_stanza ~dir ~ext_lib ~ext_obj (conf : Dune_file.Library.t) =
  ...
  let gen_archive_file ~dir ext =
    Path.relative dir (Lib_name.Local.to_string lib_name ^ ext)
  in
  ...
  in
  let jsoo_runtime =
    List.map conf.buildable.js_of_ocaml.javascript_files ~f:(Path.relative dir)
  in
  let status =
  ...
  let virtual_ =
    Option.map conf.virtual_modules ~f:(fun _ ->
        { Virtual.modules = Virtual.Modules.Unexpanded
