...
let lib_files_alias ~dir ~name ~exts =
  Alias.make
    (sprintf "lib-%s%s-all" (Lib_name.to_string name) (string_of_exts exts))
    ~dir

let setup_file_deps_alias t ~dir ~exts lib files =
  Super_context.add_alias_deps t
    (lib_files_alias ~dir ~name:(Library.best_name lib) ~exts)
    files
...
module L = struct
  ...
  let file_deps t libs ~exts = List.rev_map libs ~f:(file_deps_of_lib t ~exts)
end
