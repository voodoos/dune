...
let run t ~dir cmd =
  ...
  let exit_code =
    Printf.ksprintf Sys.command "cd %s && %s > %s 2> %s" (Filename.quote dir)
      cmd (Filename.quote stdout_fn) (Filename.quote stderr_fn)
  in
...
let create ?dest_dir ?ocamlc ?(log = ignore) name =
  ...
  let ocamlc_config =
    let ocamlc_config_output =
      run_capture_exn t ~dir:dest_dir ocamlc_config_cmd |> String.split_lines
    in
...
let compile_and_link_c_prog t ?(c_flags = []) ?(link_flags = []) code =
  ...
  let run_ok args =
    run_ok t ~dir
      (String.concat ~sep:" " (t.c_compiler :: List.map args ~f:Filename.quote))
  in
  let ok =
    if need_to_compile_and_link_separately t then
      ...
      run_ok
        (List.concat
           [c_flags; ["-I"; t.stdlib_dir; "-o"; exe_fname; c_fname]; link_flags])
  in
...
let find_in_path t prog =
  ...
  logf t "-> %s"
    (match x with None -> "not found" | Some fn -> "found: " ^ quote fn);
  x
    ...
    let env =
      ...
      | Some "macosx" -> (
        ...
        | Some brew ->
            let prefix =
              String.trim
                (run_capture_exn c ~dir (command_line brew ["--prefix"]))
            in
            sprintf
              "env PKG_CONFIG_PATH=%s/opt/%s/lib/pkgconfig:$PKG_CONFIG_PATH "
        ...
        match
          String.trim
            (run_capture_exn c ~dir
               (sprintf "%s%s %s %s" env pkg_config what package))
        with
        | "" -> []
        | s -> String.split s ~on:' '
      in
      Some {libs = run "--libs"; cflags = run "--cflags"}
    else None
end
