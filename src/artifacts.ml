  ...
  let local_bins =
    List.fold_left l ~init:String.Map.empty ~f:(fun acc x ->
        List.fold_left (f x) ~init:acc ~f:(fun local_bins stanza ->
            ...
            | _ -> local_bins ) )
  in
  {context; local_bins; public_libs}
...
let binary t ?hint ~loc name =
  if not (Filename.is_relative name) then
    ...
    | None -> (
      ...
      | None ->
          Error
            { Action.Prog.Not_found.program = name
            ; hint
...
let file_of_lib t ~loc ~lib ~file =
  ...
  | Error reason ->
      Error
        { fail =
            (fun () ->
              Lib.not_available ~loc reason "Public library %a"
                Lib_name.pp_quoted lib ) }
  | Ok lib ->
      ...
      else Ok (Path.relative (Lib.src_dir lib) file)
