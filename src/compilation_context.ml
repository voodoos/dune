  ...
  let make sctx ~opaque ~requires : _ Cm_kind.Dict.t =
    ...
    | Ok libs ->
        ...
        let cmi_includes =
          Arg_spec.S
            [ iflags
            ; Hidden_deps (Lib_file_deps.L.file_deps sctx libs ~exts:[".cmi"])
            ]
        in
        let cmx_includes =
          Arg_spec.S
            ...
            ; Hidden_deps
                ( if opaque then
                  List.map libs ~f:(fun lib ->
                      ( lib
                      , if Lib.is_local lib then [".cmi"] else [".cmi"; ".cmx"]
                      ) )
                  |> Lib_file_deps.L.file_deps_with_exts sctx
                else Lib_file_deps.L.file_deps sctx libs ~exts:[".cmi"; ".cmx"]
                ) ]
        in
        {cmi = cmi_includes; cmo = cmi_includes; cmx = cmx_includes}

  let empty = Cm_kind.Dict.make_all (Arg_spec.As [])
end
...
let create ~super_context ~scope ~dir ?private_obj_dir
    ...
    ?(dir_kind = File_tree.Dune_file.Kind.Dune) ?(obj_dir = dir) ~modules
    ?alias_module ?lib_interface_module ~flags ~requires
    ?(preprocessing = Preprocessing.dummy) ?(no_keep_locs = false) ~opaque
    ?stdlib () =
  { super_context
  ; scope
  ; dir
...
let for_wrapped_compat t modules =
  { t with
    includes = Includes.empty; alias_module = None; stdlib = None; modules }
