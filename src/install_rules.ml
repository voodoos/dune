...
module Gen (P : Params) = struct
  ...
  let init_meta () =
    ...
    |> Package.Name.Map.iter ~f:(fun ((pkg : Package.t), libs) ->
           ...
           SC.on_load_dir sctx ~dir:path ~f:(fun () ->
               ...
               let meta_template =
                 Path.extend_basename meta ~suffix:".template"
               in
               let version =
  ...
  let lib_ppxs ~(lib : Dune_file.Library.t) ~scope ~dir_kind =
    ...
    | Ppx_rewriter -> (
        ...
        | Dune -> [Preprocessing.get_compat_ppx_exe sctx ~name ~kind:Dune]
        | Jbuild ->
            let driver =
              let deps =
              ...
              | l -> (
                ...
                with
                | Named "ppxlib", _ | _, true -> Some "ppxlib.runner"
                | _ -> Some "ppx_driver.runner" )
            in
  ...
  let lib_install_files ~dir_contents ~dir ~sub_dir ~scope ~dir_kind
    ...
    let make_entry section ?dst fn =
      Install.Entry.make section fn
        ~dst:
          (let dst = match dst with Some s -> s | None -> Path.basename fn in
           match sub_dir with
           | None -> dst
      ...
      List.concat_map installable_modules ~f:(fun m ->
          List.map (Module.sources m) ~f:(fun source ->
              (* We add the -gen suffix to a few files generated by dune, such
                 as the alias module. *)
              let dst =
                Path.basename source |> String.drop_suffix ~suffix:"-gen"
              in
    ...
    let module_files =
      ...
      List.concat_map installable_modules ~f:(fun m ->
          List.concat
            [ if_ (Module.is_public m) [Module.cm_file_unsafe m ~obj_dir Cmi]
            ; if_
                (native && Module.has_impl m)
                [Module.cm_file_unsafe m ~obj_dir Cmx]
            ; if_
                (native && Module.has_impl m && virtual_library)
    ...
    List.concat
      ...
      ; [ make_entry Lib
            (lib_dune_file ~dir ~name:(Dune_file.Library.best_name lib)) ] ]

  let is_odig_doc_file fn =
    List.exists ["README"; "LICENSE"; "CHANGE"; "HISTORY"] ~f:(fun prefix ->
        String.is_prefix fn ~prefix )

  let local_install_rules (entries : Install.Entry.t list) ~install_paths
      ~package =
  ...
  let install_file (package : Package.t) entries =
    ...
    let fn =
      Path.relative
        (Path.append ctx.build_dir package.path)
    ...
    SC.add_rule sctx
      ~mode:
        ( if promote_install_file then Promote_but_delete_on_clean
        else
  ...
  let init_install () =
      ...
      List.concat_map (SC.stanzas_to_consider_for_install sctx)
        ~f:(fun {SC.Installable.dir; stanza; kind = dir_kind; scope} ->
          ...
          | Library ({public = Some {package; sub_dir; name = _}; _} as lib) ->
              List.map
                (lib_install_files ~dir ~sub_dir lib ~scope ~dir_kind
                   ~dir_contents) ~f:(fun x -> (package.name, x) )
    ...
    Package.Name.Map.iter (SC.packages sctx) ~f:(fun (pkg : Package.t) ->
        let stanzas =
          Option.value
            (Package.Name.Map.find entries_per_package pkg.name)
            ~default:[]
        in
    ...
    if not ctx.implicit then
      Package.Name.Map.iteri (SC.packages sctx)
        ~f:(fun pkg {Package.path = src_path; _} ->
          ...
          SC.add_alias_deps sctx install_alias
