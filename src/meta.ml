...
module Parse = struct
  ...
  let rec entries lb depth acc =
    ...
    | Eof ->
        if depth = 0 then List.rev acc
        else error lb "%d closing parentheses missing" depth
    | Name "package" ->
...
let load p ~name =
  { name
  ; entries = Io.with_lexbuf_from_file p ~f:(fun lb -> Parse.entries lb 0 [])
  }
  |> simplify
...
let builtins ~stdlib_dir ~version:ocaml_version =
  ...
  in
  let dummy name =
    {name = Some (Lib_name.of_string_exn ~loc:None name); entries = [version]}
  in
  let compiler_libs =
    let sub name deps =
  ...
  in
  let libs =
    let base = [compiler_libs; str; unix; bigarray; threads; dynlink; bytes] in
    let base =
...
let pp_quoted_value var =
  match var with
  | "archive" | "plugin" | "requires" | "ppx_runtime_deps" | "linkopts"
   |"jsoo_runtime" ->
      pp_print_text
  | _ -> pp_print_string
