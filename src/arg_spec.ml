...
let rec add_deps ts set =
  List.fold_left ts ~init:set ~f:(fun set t ->
      ...
      | Deps fns | Hidden_deps fns -> Path.Set.union set (Path.Set.of_list fns)
      | S ts | Concat (_, ts) -> add_deps ts set
      | _ -> set )
...
let rec add_targets ts acc =
  List.fold_left ts ~init:acc ~f:(fun acc t ->
      ...
      | S ts | Concat (_, ts) -> add_targets ts acc
      | _ -> acc )
...
let of_result = function Ok x -> x | Error e -> Dyn (fun _ -> raise e)

let of_result_map res ~f =
  match res with Ok x -> f x | Error e -> Dyn (fun _ -> raise e)
