...
let executables_rules ~sctx ~dir ~dir_kind ~dir_contents ~scope ~compile_info
  ...
  in
  let pp =
    Preprocessing.make sctx ~dir ~dep_kind:Required ~scope
      ~preprocess:exes.buildable.preprocess ~preprocessor_deps
      ~lint:exes.buildable.lint ~lib_name:None ~dir_kind
  in
  let modules =
    ...
    List.map exes.names ~f:(fun (loc, name) ->
        ...
        | Some m ->
            if not (Module.has_impl m) then
              Errors.fail loc "Module %a has no implementation." Module.Name.pp
                mod_name
            else {Exe.Program.name; main_module_name = mod_name; loc}
        | None ->
            Errors.fail loc "Module %a doesn't exist." Module.Name.pp mod_name
    )
  in
  let linkages =
    ...
    then Exe.Linkage.custom :: l
    else l
  in
  let flags = SC.ocaml_flags sctx ~scope ~dir exes.buildable in
  let link_deps = SC.Deps.interpret sctx ~scope ~dir exes.link_deps in
  let link_flags =
    link_deps >>^ ignore
    >>> SC.expand_and_eval_set sctx exes.link_flags ~scope ~dir
          ~standard:(Build.return [])
  in
  let cctx =
    Compilation_context.create () ~super_context:sctx ~scope ~dir ~dir_kind
      ~obj_dir ~modules ~flags ~requires ~preprocessing:pp
...
let rules ~sctx ~dir ~dir_contents ~scope ~dir_kind
  ...
  SC.Libs.with_lib_deps sctx compile_info ~dir ~f:(fun () ->
      executables_rules exes ~sctx ~dir ~dir_contents ~scope ~compile_info
        ~dir_kind )
