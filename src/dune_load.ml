  ...
  let eval {dune_files; ignore_promoted_rules} ~(context : Context.t) =
    ...
    Fiber.parallel_map dynamic ~f:(fun {dir; file; project; kind} ->
        let generated_dune_file =
          Path.append
            (Path.relative generated_dune_files_dir context.name)
            file
        in
        ...
        if not (Path.exists generated_dune_file) then
          die
            "@{<error>Error:@} %s failed to produce a valid dune_file file.\n\
      ...
      let dune_file =
        Dune_files.Literal
          (Dune_file.parse p.sexps ~dir ~file:p.path ~project
             ~kind:dune_file.kind ~ignore_promoted_rules)
      in
      p.sexps <- [];
      dune_file
...
let load ?extra_ignored_subtrees ?(ignore_promoted_rules = false) () =
  ...
  let rec walk dir dune_files =
    if File_tree.Dir.ignored dir then dune_files
    else
