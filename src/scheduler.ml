    ...
    let rec loop () =
      ...
      | None -> (
        ...
        | fns ->
            ...
            in
            if only_ignored_files then loop () else Files_changed )
    in
    let ev = loop () in
    Mutex.unlock mutex;
    ev
    ...
    lazy
      (let excludes =
       ...
       | None -> (
         ...
         | None ->
             die
               "@{<error>Error@}: fswatch (or inotifywait) was not found. One \
                of them needs to be installed for watch mode to work.\n" ))

  let buffering_time = 0.5
...
module Process_watcher : sig
  ...
  module Process_table : sig
    ...
    let remove ~pid status =
      ...
      | None -> Hashtbl.add table pid (Zombie status)
      | Some (Running job) ->
          decr running_count;
...
let show_status_line s = prerr_string s
...
let set_concurrency n = Fiber.Var.get_exn t_var >>| fun t -> t.concurrency <- n

let wait_for_available_job () =
  Fiber.Var.get_exn t_var
  >>= fun t ->
  if Event.pending_jobs () < t.concurrency then Fiber.return t
  else
...
let rec restart_waiting_for_available_job t =
  ...
  then Fiber.return ()
    ...
    Fiber.Ivar.fill ivar t >>= fun () -> restart_waiting_for_available_job t
  ...
  let rec go_rec t =
    ...
    if count = 0 then (
      ...
      | Job_completed (job, status) ->
          ...
          >>= fun () ->
          restart_waiting_for_available_job t >>= fun () -> go_rec t
      | Files_changed ->
...
let run t fiber =
  ...
  in
  Fiber.run (Fiber.fork_and_join_unit (fun () -> go_rec t) (fun () -> fiber))
...
let poll ?log ?config ~once ~finally ~canceled () =
  ...
  let rec main_loop () =
    ...
    wait_success ()
    >>= function Exit -> Fiber.return Got_signal | Continue -> main_loop ()
  in
  let continue_on_error () =
    if not t.cur_build_canceled then (
      finally ();
      wait_failure () >>= fun _ -> main_loop () )
    else
      set_status_line_generator (fun () ->
      ...
      Process_watcher.killall Sys.sigkill;
      let rec loop () =
        if Event.pending_jobs () = 0 then Continue
        else
          match Event.next () with
          | Files_changed -> loop ()
