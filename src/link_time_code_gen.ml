...
let libraries_link ~name ~loc ~mode cctx libs =
  ...
  | Some (before, after) ->
      ...
      let s =
        Format.asprintf "%a@\nFindlib.record_package_predicates %a;;@."
          (Fmt.list ~pp_sep:Fmt.nl (fun fmt lib ->
               Format.fprintf fmt
                 "Findlib.record_package Findlib.Record_core %a;;"
      ...
      let cctx =
        Compilation_context.(
          create ~super_context:sctx ~scope:(scope cctx) ~dir:(dir cctx)
            ~dir_kind:(dir_kind cctx) ~obj_dir:(obj_dir cctx)
            ...
            ~flags:Ocaml_flags.empty ~opaque:true ())
      in
      ...
      let lm =
        of_libs before
        @ [Lib.Lib_and_module.Module (module_, obj_dir)]
        @ of_libs after
      in
      Arg_spec.S
        [A "-linkall"; Lib.Lib_and_module.link_flags lm ~mode ~stdlib_dir]
  | None -> Lib.L.link_flags libs ~mode ~stdlib_dir
