...
module Entry = struct
  ...
  let compare x y =
    ...
    if c <> Eq then c
      ...
      if c <> Eq then c else Section.compare x.section y.section

  let make section ?dst src =
    let dst =
      if Sys.win32 then
        let src_base = Path.basename src in
        let dst' = match dst with None -> src_base | Some s -> s in
        match Filename.extension src_base with
        | ".exe" | ".bc" ->
            if Filename.extension dst' <> ".exe" then Some (dst' ^ ".exe")
            else dst
        | _ -> dst
      else dst
    in
      ...
      | Some section -> (
        ...
        | List (_, l) ->
            List.map l ~f:(function
              | String (_, src) ->
                  {Entry.src = Path.of_string src; dst = None; section}
              | Option (_, String (_, src), [String (_, dst)]) ->
                  {Entry.src = Path.of_string src; dst = Some dst; section}
              | v ->
