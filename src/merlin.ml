...
let make ?(requires = Ok []) ?(flags = Build.return [])
    ?(preprocess = Dune_file.Preprocess.No_preprocessing) ?libname
    ?(source_dirs = Path.Set.empty) ?(objs_dirs = Path.Set.empty) () =
...
let add_source_dir t dir = {t with source_dirs = Path.Set.add t.source_dirs dir}

let ppx_flags sctx ~dir:_ ~scope ~dir_kind {preprocess; libname; _} =
  match preprocess with
  | Pps {loc = _; pps; flags; staged = _} -> (
