...
module Driver = struct
  module M = struct
    ...
    let instantiate ~resolve ~get lib (info : Info.t) =
      ...
      ; replaces =
          ...
          Result.List.map info.replaces ~f:(fun ((loc, name) as x) ->
              ...
              | None ->
                  Error
                    (Errors.exnf loc "%a is not a %s" Lib_name.pp_quoted name
                       (desc ~plural:false))
              | Some t -> Ok t )) }
    ...
    let encode t =
      ...
      ( (1, 0)
      , record
          [ ("flags", Ordered_set_lang.Unexpanded.encode t.info.flags)
          ; ("lint_flags", Ordered_set_lang.Unexpanded.encode t.info.lint_flags)
  ...
  let make_error loc msg =
    ...
    | Dot_ppx (path, pps) ->
        Error
          (Errors.exnf
             (Loc.in_file (Path.to_string path))
             "%a" Fmt.text
             (sprintf "Failed to create on-demand ppx rewriter for %s; %s"
...
module Jbuild_driver = struct
  ...
  let make name info : (Lib_name.t * Driver.t) Lazy.t =
    lazy
      (let info =
       ...
       , {info; lib = lazy (assert false); replaces = Ok []} ))

  let omp =
    make "ocaml-migrate-parsetree"
      {|
  ...
  let drivers =
    [ ( Lib_name.of_string_exn ~loc:None "ocaml-migrate-parsetree.driver-main"
      , omp )
...
let ppx_exe sctx ~key ~dir_kind =
  ...
  | Dune -> Path.relative (SC.build_dir sctx) (".ppx/" ^ key ^ "/ppx.exe")
  | Jbuild ->
...
let build_ppx_driver sctx ~lib_db ~dep_kind ~target ~dir_kind pps =
  ...
  SC.add_rule sctx
    ( Build.record_lib_deps (Lib_deps.info ~kind:dep_kind (Lib_deps.of_pps pps))
    ...
    >>> Build.run (Ok compiler) ~dir:ctx.build_dir
          ...
          ; Arg_spec.of_result
              (Result.map driver_and_libs ~f:(fun (_driver, libs) ->
                   Lib.L.compile_and_link_flags ~mode
                     ~stdlib_dir:ctx.stdlib_dir ~compile:libs ~link:libs ))
          ; Dep ml ] )
...
let get_rules sctx key ~dir_kind =
  ...
  let names =
    match key with "+none+" -> [] | _ -> String.split key ~on:'+'
  in
  let names =
...
let get_ppx_driver sctx ~loc ~scope ~dir_kind pps =
  ...
  | Jbuild -> Ok (Jbuild_driver.get_driver pps) )
  >>| fun driver -> (ppx_driver_exe (SC.host sctx) libs ~dir_kind, driver)
...
let setup_reason_rules sctx (m : Module.t) =
  let ctx = SC.context sctx in
  let refmt =
    SC.resolve_program sctx ~loc:None "refmt" ~hint:"try: opam install reason"
  in
  let rule src target =
    Build.run ~dir:ctx.build_dir refmt
      [A "--print"; A "binary"; Dep src]
      ~stdout_to:target
  in
            ...
            let suffix =
              ...
              | _ ->
                  Errors.fail
                    ...
                    "Unknown file extension for reason source file: %S" ext
            in
  ...
  Staged.stage
    (let alias = Build_system.Alias.lint ~dir in
     let add_alias fn build =
       SC.add_alias_action sctx alias build ~stamp:("lint", lib_name, fn)
     in
     let lint =
       Per_module.map lint ~f:(function
         ...
         | Action (loc, action) ->
             fun ~source ~ast:_ ->
               let action =
                 Action.Unexpanded.Chdir (workspace_root_var, action)
               in
               Module.iter source ~f:(fun _ (src : Module.File.t) ->
                   ...
                   add_alias src.path ~loc:(Some loc)
                     ...
                     >>> SC.Action.run sctx action ~loc ~dir ~dep_kind
                           ~bindings ~targets:(Static []) ~targets_dir:dir
                           ~scope ) )
         | Pps {loc; pps; flags; staged} ->
             ...
             let args : _ Arg_spec.t =
               S [As flags; As (cookie_library_name lib_name)]
             in
             ...
             let driver_and_flags =
               ...
               , let bindings =
                 ...
                 Build.memoize "ppx flags"
                   (SC.expand_and_eval_set sctx driver.info.lint_flags ~scope
                      ~dir ~bindings ~standard:(Build.return [])) )
             in
             fun ~source ~ast ->
...
let make sctx ~dir ~dep_kind ~lint ~preprocess ~preprocessor_deps ~lib_name
    ~scope ~dir_kind =
  ...
  let lint_module =
    Staged.unstage
      (lint_module sctx ~dir ~dep_kind ~lint ~lib_name ~scope ~dir_kind)
  in
            ...
            pped_module m ~f:(fun _kind src dst ->
                ...
                SC.add_rule sctx ~loc
                  ...
                  >>> SC.Action.run sctx
                        ...
                        ~loc ~dir ~dep_kind ~bindings
                        ~targets:(Static [dst])
                        ~targets_dir:dir ~scope ) )
        ...
        if not staged then (
          ...
          let driver_and_flags =
            ...
            , let bindings =
              ...
              in
              Build.memoize "ppx flags"
                (SC.expand_and_eval_set sctx driver.info.flags ~scope ~dir
                   ~bindings
          ...
          let pp_flags =
            Build.of_result
              ...
              Build.memoize "ppx command"
                ( Build.path exe >>> preprocessor_deps >>^ ignore
                >>> SC.expand_and_eval_set sctx driver.info.as_ppx_flags ~scope
                      ~dir ~standard:(Build.return [])
                >>^ fun flags ->
                let command =
                  List.map
                    (List.concat
