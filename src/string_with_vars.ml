...
let make ?(quoted = false) loc part =
  {template = {parts = [part]; quoted; loc}; syntax_version = (1, 0)}

let make_text ?quoted loc s = make ?quoted loc (Text s)

let make_var ?quoted loc name =
  let var = {loc; name; payload = None; syntax = Percent} in
...
module Mode = struct
  ...
  let string : type a. a t -> string -> a =
   fun t s -> match t with Single -> Value.String s | Many -> [Value.String s]
    ...
    partial_expand t ~mode ~dir ~f:(fun var syntax_version ->
        ...
        | None -> (
          ...
          | Percent ->
              ...
              else Errors.fail var.loc "Unknown variable %S" (Var.name var)
...
let text_only t = match t.template.parts with [Text s] -> Some s | _ -> None
