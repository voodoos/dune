...
module Syntax = struct
  ...
  let to_string = function OCaml -> "ocaml" | Reason -> "reason"
...
module File = struct
  ...
  let to_sexp {path; syntax} =
    ...
    record [("path", Path.to_sexp path); ("syntax", Syntax.to_sexp syntax)]

  let pp fmt {path; syntax} =
    Fmt.record fmt
      [("path", Fmt.const Path.pp path); ("syntax", Fmt.const Syntax.pp syntax)]
end
...
let make ?impl ?intf ?obj_name ~visibility name =
  ...
  in
  {name; impl; intf; obj_name; pp = None; visibility}
...
let file t (kind : Ml_kind.t) =
  let file = match kind with Impl -> t.impl | Intf -> t.intf in
...
let with_wrapper t ~main_module_name =
  { t with
    obj_name = sprintf "%s__%s" (String.uncapitalize main_module_name) t.name
  }
      ...
      Some
        ...
        ; path =
            (* Option.value_exn cannot fail because we disallow wrapped
               ...
               a source dir *)
            Path.L.relative
              (Option.value_exn (src_dir t))
              [".wrapped_compat"; Name.to_string t.name ^ ".ml-gen"] } }
...
module Name_map = struct
  ...
  let impl_only =
    Name.Map.fold ~init:[] ~f:(fun m acc ->
        if has_impl m then m :: acc else acc )

  let of_list_exn modules =
    List.map modules ~f:(fun m -> (name m, m)) |> Name.Map.of_list_exn

  let add t module_ = Name.Map.add t (name module_) module_
end
...
let set_private t = {t with visibility = Private}

let remove_files t = {t with intf = None; impl = None}
      ...
      ~deps:(fun m ->
        ...
        | None ->
            Exn.code_error "top_closure: unable to find key"
              [("m", to_sexp m); ("t", (Sexp.Encoder.list to_sexp) (keys t))]
        )
end
