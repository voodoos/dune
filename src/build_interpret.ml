...
module Target = struct
  ...
  let path = function Normal p -> p | Vfile (Vspec.T (p, _)) -> p
...
let inspect_path file_tree path =
  ...
  | None ->
      if not (Path.exists path) then None
      else if Path.is_directory path then Some Dir
      else Some Reg
  | Some path -> (
    match File_tree.find_dir file_tree path with
    | Some _ -> Some Dir
    | None ->
        if Path.is_root path then Some Dir
        else if
          File_tree.file_exists file_tree (Path.parent_exn path)
            (Path.basename path)
        then Some Reg
        else None )

let[@inline never] no_targets_allowed () =
  Exn.code_error
...
let static_deps t ~all_targets ~file_tree =
  let rec loop : type a b. (a, b) t -> Static_deps.t -> bool -> Static_deps.t =
   fun t acc targets_allowed ->
    ...
    | Vpath (Vspec.T (p, _)) -> Static_deps.add_rule_path acc p
    ...
    | Env_var var -> Static_deps.add_action_env_var acc var
  in
...
let targets =
  let rec loop : type a b. (a, b) t -> Target.t list -> Target.t list =
   fun t acc ->
    match t with
    | Arr _ -> acc
    | Targets targets ->
        List.fold_left targets ~init:acc ~f:(fun acc fn ->
            Target.Normal fn :: acc )
        ...
        | a, b ->
            ...
            Exn.code_error
              "Build_interpret.targets: cannot have targets under a \
               [if_file_exists]"
              [("targets-a", targets a); ("targets-b", targets b)] ) )
    | Memo m -> loop m.t acc
    | Catch (t, _) -> loop t acc
  ...
  let make ?(sandbox = false) ?(mode = Dune_file.Rule.Mode.Not_a_rule_stanza)
    ...
    let dir =
      ...
      | x :: l ->
              ...
              if Path.parent_exn path <> dir then
                ...
                | None ->
                    Exn.code_error "rule has targets in different directories"
                      [ ( "targets"
                        , Sexp.Encoder.list Path.to_sexp
          ...
          dir
    in
    {context; build; targets; sandbox; mode; locks; loc; dir}
end
