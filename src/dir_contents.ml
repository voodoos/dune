...
let dirs t =
  match t.kind with
  | Standalone -> [t]
  | Group_root (lazy l) | Group_part {kind = Group_root (lazy l); _} -> t :: l
...
let modules_of_library t ~name =
  ...
  | None ->
      Exn.code_error "Dir_contents.modules_of_library"
        ...
        ; ( "available"
          , Sexp.Encoder.(list Lib_name.to_sexp) (Lib_name.Map.keys map) ) ]
...
let load_text_files sctx ft_dir
    { Super_context.Dir_with_dune.ctx_dir = dir
    ; src_dir
    ; scope
    ; stanzas
    ; kind = _ } =
  ...
  let generated_files =
    List.concat_map stanzas ~f:(fun stanza ->
        ...
        | Rule rule ->
            List.map
              (Simple_rules.user_rule sctx rule ~dir ~scope)
              ~f:Path.basename
        | Copy_files def ->
            List.map
              (Simple_rules.copy_files sctx def ~src_dir ~dir ~scope)
              ~f:Path.basename
...
let modules_of_files ~dir ~files =
  let make_module syntax base fn =
    (Module.Name.of_string base, Module.File.make syntax (Path.relative dir fn))
  in
  ...
  let parse_one_set (files : (Module.Name.t * Module.File.t) list) =
    ...
    | Error (name, f1, f2) ->
        ...
        die "Too many files for module %a in %a:\n- %a\n- %a" Module.Name.pp
          name Path.pp src_dir Path.pp f1.path Path.pp f2.path
  in
...
let build_modules_map (d : Super_context.Dir_with_dune.t) ~scope ~modules =
  let libs, exes =
    List.filter_partition_map d.stanzas ~f:(fun stanza ->
        match (stanza : Stanza.t) with
        | Library lib ->
            let {Modules_field_evaluator.all_modules = modules; virtual_modules}
                =
        ...
        | Executables exes | Tests {exes; _} ->
            let { Modules_field_evaluator.all_modules = modules
                ; virtual_modules = _ } =
  ...
  let libraries =
    ...
    | Error (name, _, (lib2, _)) ->
        Errors.fail lib2.buildable.loc
          "Library %a appears for the second time in this directory"
        ...
        |> Module.Name.Map.mapi ~f:(fun name buildables ->
               ...
               | b :: rest ->
                   let locs =
                     List.sort ~compare
                       ( b.Buildable.loc
                       :: List.map rest ~f:(fun b -> b.Buildable.loc) )
                   in
                   Errors.warn
                     (Loc.in_file b.loc.start.pos_fname)
                     "Module %a is used in several stanzas:@\n\
                      @[<v>%a@]@\n\
                      @[%a@]@\n\
                     ...
                     locs Format.pp_print_text
                     "To remove this warning, you must specify an explicit \
                      \"modules\" field in every library, executable, and \
                      executables stanzas in this jbuild file. Note that each \
                      module cannot appear in more than one \"modules\" field \
    ...
    | Standalone x ->
        let t =
          ...
          | Some (ft_dir, Some d) ->
              ...
              ; modules =
                  lazy
                    (build_modules_map d ~scope:d.scope
          ...
          lazy
            (let modules =
               List.fold_left ((dir, files) :: subdirs)
                 ~init:Module.Name.Map.empty ~f:(fun acc (dir, files) ->
                   ...
                   Module.Name.Map.union acc modules ~f:(fun name x y ->
                       Errors.fail
                         ...
                         "Module %a appears in several directories:@\n\
                          - %a@\n\
                          - %a"
                         Module.Name.pp_quote name (Fmt.optional Path.pp)
                         (Module.src_dir x) (Fmt.optional Path.pp)
                         (Module.src_dir y) ) )
             in
