  ...
  let extra_env ~profile env_nodes =
    let make_env l =
      ...
      Option.value ~default:Env.empty
        ( l
        >>= fun stanza ->
        Dune_env.Stanza.find stanza ~profile >>| fun env -> env.env_vars )
    in
    Env.extend_env (make_env env_nodes.context) (make_env env_nodes.workspace)
end
...
let which ~cache ~path x = Hashtbl.find_or_add cache x ~f:(Bin.which ~path)
...
let ocamlfind_printconf_path ~env ~ocamlfind ~toolchain =
  ...
  >>| fun l -> List.map l ~f:Path.of_filename_relative_to_initial_cwd

let create ~(kind : Kind.t) ~path ~env ~env_nodes ~name ~merlin ~targets
    ~host_toolchain ~profile =
  let opam_var_cache = Hashtbl.create 128 in
  ( match kind with
  | Opam {root = Some root; _} -> Hashtbl.add opam_var_cache "root" root
  | _ -> () );
  ...
  let create_one ~name ~implicit ~findlib_toolchain ~host ~merlin =
    ...
    | None -> Fiber.return None
    | Some toolchain ->
        Lazy.force findlib_config_path
        >>| fun path ->
    ...
    let env =
      ...
      let vars =
        let local_lib_path =
          Path.relative (Config.local_install_dir ~context:name) "lib"
        in
  ...
  create_one ~host:None ~findlib_toolchain:host_toolchain ~implicit ~name
    ~merlin
  >>= fun native ->
  Fiber.parallel_map targets ~f:(function
    ...
    | Named findlib_toolchain ->
        ...
        >>| fun x -> Some x )
  >>| fun others -> native :: List.filter_map others ~f:(fun x -> x)

let opam_config_var t var =
  opam_config_var ~env:t.env ~cache:t.opam_var_cache var

let default ~merlin ~env_nodes ~env ~targets =
  create ~kind:Default ~path:Bin.path ~env ~env_nodes ~name:"default" ~merlin
    ~targets
  ...
  fun opam env ->
    ...
    | None ->
        Fiber.fork (fun () ->
            ...
            >>| fun s ->
            try Scanf.sscanf s "%d.%d.%d" (fun a b c -> (a, b, c)) with _ ->
              die
                "@{<error>Error@}: `%a config --version' returned invalid \
                 output:\n\
                 %s"
                Path.pp opam s )
        >>= fun future ->
...
let create_for_opam ~root ~env ~env_nodes ~targets ~profile ~switch ~name
    ...
    |> Env.Map.mapi ~f:(fun var values ->
           ...
           | x :: _ ->
               Format.eprintf
                 ...
                 var
                 (String.concat ~sep:" "
                    (List.map ~f:quote_for_shell (Path.to_string opam :: args)));
               x )
  in
  let path =
  ...
  create
    ...
    ~profile ~targets ~path ~env ~env_nodes ~name ~merlin ~host_toolchain

let create ~env (workspace : Workspace.t) =
  let env_nodes context = {Env_nodes.context; workspace = workspace.env} in
  Fiber.parallel_map workspace.contexts ~f:(fun def ->
      ...
      | Default {targets; profile; env = env_node; toolchain; loc = _} ->
          ...
          default ~env ~env_nodes:(env_nodes env_node) ~profile ~targets
            ~merlin ~host_toolchain
      | Opam
          { base = {targets; profile; env = env_node; toolchain; loc = _}
          ; name
          ; switch
          ; root
          ; merlin } ->
...
let install_ocaml_libdir t =
  ...
  | _ -> (
    ...
    | Some fn ->
        ...
        >>| fun s -> Some (Path.of_filename_relative_to_initial_cwd s)
    | None -> Fiber.return None )
...
let cc_g (ctx : t) = if ctx.ccomp_type <> "msvc" then ["-g"] else []
