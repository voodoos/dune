...
module Promoted_to_delete = struct
  ...
  let dump () =
    if Path.build_dir_exists () then load () |> Path.Set.union !db |> P.dump fn
end
...
module Internal_rule = struct
  ...
  let dependency_cycle ~last ~last_rev_dep ~last_requested_file =
    let rec build_loop acc t =
      if t.id = last.id then last_requested_file :: acc
      else
        let requested_file, rev_dep =
          List.find_exn t.rev_deps ~f:(fun (_, t) ->
  ...
  let push_rev_dep t requested_file ~f =
    ...
    let on_error exn = Dep_path.reraise exn (Path requested_file) in
...
module File_kind = struct
  ...
  let eq : type a b. a t -> b t -> (a, b) Type_eq.t option =
   fun a b ->
    match (a, b) with
...
module File_spec = struct
  ...
  let create rule kind = T {rule; kind; data = None}
end
...
module Alias0 = struct
  module T : sig
    ...
    let make name ~dir =
      if (not (Path.is_in_build_dir dir)) || String.contains name '/' then
        Exn.code_error "Alias0.make: Invalid alias"
          [("name", Sexp.Encoder.string name); ("dir", Path.to_sexp dir)];
      {dir; name}
    ...
    let of_user_written_path ~loc path =
      ...
      {dir = Path.parent_exn path; name = Path.basename path}
  end
  ...
  let stamp_file t =
    Path.relative
      (Path.insert_after_build_dir_exn t.dir ".aliases")
      (t.name ^ suffix)
...
module Dir_status = struct
  type waiting_for_load_dir = {mutable lazy_generators : (unit -> unit) list}
...
module Trace = struct
  ...
  let dump t = if Path.build_dir_exists () then P.dump file t

  let load () =
    match P.load file with Some t -> t | None -> Path.Table.create 1024
end
  ...
  Path.Table.find_or_add t.dirs dir ~f:(fun _ ->
      if Path.is_in_source_tree dir then
        ...
        Dir_status.Loaded
          ...
          | files -> Path.Set.of_list (List.map files ~f:(Path.relative dir))
          )
      else
        let ctx, sub_dir = Option.value_exn (Path.extract_build_context dir) in
        if ctx = ".aliases" then Forward (Path.(append build_dir) sub_dir)
...
module Build_exec = struct
  ...
  let exec bs t x =
    let rec exec : type a b. Deps.t ref -> (a, b) t -> a -> b =
     fun dyn_deps t x ->
      match t with
      | Arr f -> f x
      | Targets _ -> x
  ...
  | Some (File_spec.T {rule; _}) -> (
    ...
    | true, (Standard | Not_a_rule_stanza) ->
        Errors.warn
          ...
          (fun ppf ->
            ...
            | Standard ->
                Format.fprintf ppf
                  "To keep the current behavior and get rid of this warning, \
                   add a field (fallback) to the rule."
            | _ -> assert false );
    ...
    | _ ->
        ...
        die "Multiple rules generated for %s:\n- %s\n- %s"
          ...
          ( if copy_source then "<internal copy rule>"
          else string_of_loc rule.loc )
...
let make_local_dir t fn =
  if not (Path.Set.mem t.local_mkdirs fn) then (
    Path.mkdir_p fn;
    t.local_mkdirs <- Path.Set.add t.local_mkdirs fn )

let make_local_dirs t paths = Path.Set.iter paths ~f:(make_local_dir t)
...
let remove_old_artifacts t ~dir ~subdirs_to_keep =
  if
    (not (Path.is_in_build_dir dir))
    || Path.Table.mem t.files (Path.relative dir Config.dune_keep_fname)
  then ()
  else
    match Path.readdir_unsorted dir with
    | exception _ -> ()
    | files ->
        List.iter files ~f:(fun fn ->
            ...
            | {st_kind = S_DIR; _} -> (
              ...
              | These set ->
                  if
                    String.Set.mem set fn
                    || Path.Set.mem t.build_dirs_to_keep path
                  then ()
                  else Path.rm_rf path )
            | exception _ ->
                if not (Path.Table.mem t.files path) then Path.unlink path
            | _ -> if not (Path.Table.mem t.files path) then Path.unlink path
        )
...
let parallel_iter_deps deps ~f = parallel_iter_paths (Deps.paths deps) ~f

let rec compile_rule t ?(copy_source = false) pre_rule =
  let { Pre_rule.context
      ; build
  ...
  let exec_rule (rule_evaluation : Exec_status.rule_evaluation) =
    ...
    let env = match context with None -> Env.empty | Some c -> c.env in
    ...
    in
    let force =
      !Clflags.force && List.exists targets_as_list ~f:Path.is_alias_stamp_file
    in
        ...
        Path.Set.iter targets ~f:(fun path ->
            ...
            if
              ...
              || Utils.Cached_digest.file path
                 <> Utils.Cached_digest.file in_source_tree
            then (
...
and load_dir_and_get_targets t ~dir =
  ...
  | Collecting_rules collector -> (
      ...
      try load_dir_step2_exn t ~dir ~collector ~lazy_generators with exn ->
...
and load_dir_step2_exn t ~dir ~collector ~lazy_generators =
  ...
  let context_name, sub_dir =
    Option.value_exn (Path.extract_build_context dir)
  in
  (* Load all the rules *)
  let extra_subdirs_to_keep =
    if context_name = "install" then These String.Set.empty
    else
      let gen_rules =
        Option.value_exn (String.Map.find t.gen_rules context_name)
      in
  ...
  let alias_rules, alias_stamp_files =
    ...
    let aliases =
      if String.Map.mem collector.aliases "default" then aliases
      else
    ...
    String.Map.foldi aliases ~init:([], Path.Set.empty)
      ~f:(fun name
         {Dir_status.deps; dyn_deps; actions}
  ...
  let targets, to_copy, subdirs_to_keep =
    ...
    | "install" -> (user_rule_targets, None, String.Set.empty)
    | ctx_name ->
        ...
        if Path.Set.is_empty files then (user_rule_targets, None, subdirs)
        else
    ...
    | Some (_, to_copy) ->
        List.filter rules ~f:(fun (rule : Build_interpret.Rule.t) ->
            ...
            | Standard | Promote | Promote_but_delete_on_clean
             |Not_a_rule_stanza | Ignore_source_files ->
                true
            | Fallback ->
                ...
                then (* All targets are present *)
                  ...
                  Path.Set.is_empty
                    (Path.Set.inter source_files_for_targtes to_copy)
...
and wait_for_file t ~loc fn =
  ...
  | None ->
      ...
      else if Path.exists fn then Fiber.return ()
      else die "File unavailable: %s" (Path.to_string_maybe_quoted fn)
...
and wait_for_file_found fn (File_spec.T file) =
  Internal_rule.push_rev_dep file.rule fn ~f:(fun () ->
      ...
      | Not_started {eval_rule; exec_rule} ->
          ...
          >>= fun rule_execution ->
          (file.rule).exec <- Running {rule_evaluation; rule_execution};
          Fiber.Future.wait rule_execution
      | Running {rule_execution; _} -> Fiber.Future.wait rule_execution
...
and wait_for_deps ~loc t deps = wait_for_path_deps ~loc t (Deps.paths deps)

let stamp_file_for_files_of t ~dir ~ext =
  let files_of_dir =
    Path.Table.find_or_add t.files_of dir ~f:(fun dir ->
        let files_by_ext =
          targets_of t ~dir |> Path.Set.to_list
      ...
      compile_rule t
        ...
        Pre_rule.make ~context:None
          ...
          >>> Build.action ~targets:[stamp_file]
                (Action.with_stdout_to stamp_file (Action.digest_files files))
          ));
...
let create ~contexts ~file_tree ~hook =
  ...
  let t =
    ...
    ; file_tree
    ; gen_rules =
        String.Map.map contexts ~f:(fun _ ~dir:_ ->
...
let eval_request t ~request ~process_target =
  ...
  Fiber.fork_and_join_unit
    ...
    (fun () ->
      ...
      parallel_iter_paths ~f:process_target
        (Deps.path_diff dyn_deps static_deps)
      >>| fun () -> result )
...
let update_universe t =
  ...
  let n =
    if Path.exists universe_file then
      Dune_lang.Decoder.(parse int)
        Univ_map.empty
        (Dune_lang.Io.load ~mode:Single universe_file)
      + 1
    else 0
  in
  make_local_dirs t (Path.Set.singleton Path.build_dir);
  Io.write_file universe_file
    (Dune_lang.to_string ~syntax:Dune (Dune_lang.Encoder.int n))
...
let rules_for_files t paths =
  Path.Set.fold paths ~init:[] ~f:(fun path acc ->
      if Path.is_in_build_dir path then load_dir t ~dir:(Path.parent_exn path);
      match Path.Table.find t.files path with
      | None -> acc
      | Some (File_spec.T {rule; _}) -> rule :: acc )
  |> Ir_set.of_list |> Ir_set.to_list
  ...
  | Error cycle ->
      die "dependency cycle detected:\n   %s"
        ( List.map cycle ~f:(fun rule ->
              Path.to_string
                (Option.value_exn (Path.Set.choose rule.Internal_rule.targets))
          )
        |> String.concat ~sep:"\n-> " )
...
let build_rules_internal ?(recursive = false) t ~request =
  ...
  let rec loop fn =
    ...
    match Path.Table.find t.files fn with
    | Some file -> file_found fn file
    | None -> Fiber.return ()
  and file_found fn (File_spec.T {rule = ir; _}) =
    if Rule.Id.Set.mem !rules_seen ir.id then Fiber.return ()
    else (
      ...
      Internal_rule.push_rev_dep ir fn ~f:(fun () ->
          ...
          | Not_started {eval_rule; exec_rule} ->
              Fiber.fork eval_rule
              >>| fun rule_evaluation ->
              ir.exec <- Evaluating_rule {rule_evaluation; exec_rule};
              rule_evaluation )
      >>= fun rule_evaluation ->
      Fiber.fork (fun () ->
          Fiber.Future.wait rule_evaluation
          >>| fun (action, dyn_deps) ->
          let action_deps =
            Static_deps.action_deps (Lazy.force ir.static_deps)
          in
          { Rule.id = ir.id
          ; dir = ir.dir
      ...
      >>= fun rule ->
      rules := rule :: !rules;
      if not recursive then Fiber.return ()
      else
        Fiber.Future.wait rule
        >>= fun rule -> parallel_iter_deps rule.deps ~f:loop )
  in
...
let set_package t file package = Path.Table.add t.packages file package
...
let package_deps t pkg files =
  ...
  let rec loop fn acc =
    ...
    | [] -> loop_deps fn acc
    | pkgs ->
        if List.mem pkg ~set:pkgs then loop_deps fn acc
        else List.fold_left pkgs ~init:acc ~f:add_package
  and add_package acc p =
    let open Package.Name.Infix in
    if p = pkg then acc else Package.Name.Set.add acc p
  and loop_deps fn acc =
    match Path.Table.find t.files fn with
    | None -> acc
    | Some (File_spec.T {rule = ir; _}) ->
        if Rule.Id.Set.mem !rules_seen ir.id then acc
        else (
          rules_seen := Rule.Id.Set.add !rules_seen ir.id;
          let _, dyn_deps =
            match ir.exec with
...
let rec add_build_dir_to_keep t ~dir =
  if not (Path.Set.mem t.build_dirs_to_keep dir) then (
    t.build_dirs_to_keep <- Path.Set.add t.build_dirs_to_keep dir;
    Option.iter (Path.parent dir) ~f:(fun dir ->
        if not (Path.is_root dir) then add_build_dir_to_keep t ~dir ) )
...
let get_collector t ~dir =
  ...
  | Loaded _ | Forward _ ->
      Exn.code_error
        ...
        else "Build_system.get_collector called on closed directory" )
...
let add_rule t (rule : Build_interpret.Rule.t) =
  let rule =
    match t.prefix with
    | None -> rule
    | Some prefix -> {rule with build = Build.O.( >>> ) prefix rule.build}
  in
...
let prefix_rules t prefix ~f =
  ...
  let prefix =
    match t.prefix with None -> prefix | Some p -> Build.O.( >>> ) p prefix
  in
...
let on_load_dir t ~dir ~f =
  ...
  | Pending p ->
      ...
      if
        lazy_generators = [] && collector.rules = []
        && String.Map.is_empty collector.aliases
      then add_build_dir_to_keep t ~dir;
...
let eval_glob t ~dir re =
  Path.Set.fold (targets_of t ~dir) ~init:[] ~f:(fun path acc ->
      ...
      if Re.execp re fn then fn :: acc else acc )
  |> List.rev
...
module Alias = struct
  ...
  let get_alias_def build_system t =
    ...
    | None ->
        let x =
          { Dir_status.deps = Path.Set.empty
  ...
  let add_deps build_system t ?dyn_deps deps =
    ...
    | Some build ->
        let open Build.O in
        def.dyn_deps
        <- ( Build.fanout def.dyn_deps build
