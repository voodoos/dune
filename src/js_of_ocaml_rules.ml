...
let runtime_file ~sctx file =
  match
    Artifacts.file_of_lib (SC.artifacts sctx) ~loc:Loc.none
...
let link_rule cc ~runtime ~target =
  ...
  in
  let jsoo_link =
    SC.resolve_program sctx ~loc:None ~hint:install_jsoo_hint "jsoo_link"
  in
  Build.run
    ~dir:(Compilation_context.dir cc)
    jsoo_link
...
let build_exe cc ~js_of_ocaml ~src =
  ...
  let javascript_files =
    List.map javascript_files ~f:(Path.relative (Compilation_context.dir cc))
  in
  ...
  else [exe_rule cc ~javascript_files ~src ~target]
