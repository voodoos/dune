...
let bash_exn =
  ...
  fun ~needed_to ->
    match Lazy.force bin with
    | Some path -> path
    | None -> die "I need bash to %s but I couldn't find it :(" needed_to
...
let signal_name =
  ...
  in
  fun n ->
    match List.assoc table n with None -> sprintf "%d\n" n | Some s -> s
...
let analyse_target fn =
  ...
  | Some (".aliases", sub) -> (
    ...
    | None -> Other fn
    | Some (ctx, fn) ->
        if Path.is_root fn then Other fn
        else
          let basename =
...
let describe_target fn =
  ...
  | Other fn -> Path.to_string_maybe_quoted fn
...
let line_directive ~filename:fn ~line_number =
  let directive =
    match Filename.extension fn with ".c" | ".cpp" | ".h" -> "line" | _ -> ""
  in
...
module Persistent (D : Persistent_desc) = struct
  ...
  let to_out_string (v : D.t) = magic ^ Marshal.to_string v []
  ...
  let load file =
    if Path.exists file then
      Io.with_file_in file ~f:(fun ic ->
          ...
          | s ->
              if s = magic then Some (Marshal.from_channel ic : D.t) else None
      )
    else None
end
...
module Cached_digest = struct
  ...
  let cache = {checked_key = 0; table = Hashtbl.create 1024}
  ...
  let file fn =
    ...
    | Some x ->
        if x.timestamp_checked = cache.checked_key then x.digest
        else
          let mtime = (Unix.stat (Path.to_string fn)).st_mtime in
          if mtime <> x.timestamp then (
            let digest = Digest.file (Path.to_string fn) in
            x.digest <- digest;
            x.timestamp <- mtime );
  ...
  let dump () = if Path.build_dir_exists () then P.dump db_file cache
