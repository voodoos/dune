    ...
    let stanza =
      ...
      let sub_dir =
        plain_string (fun ~loc dn ->
            ...
            then of_sexp_errorf loc "Invalid sub-directory name %S" dn
            else dn )
      in
      sum [("ignored_subdirs", list sub_dir >>| String.Set.of_list)]
    in
    fun sexps ->
...
let load_jbuild_ignore path =
  List.filteri (Io.lines_of_file path) ~f:(fun i fn ->
      if Filename.dirname fn = Filename.current_dir_name then true
      else (
        Errors.(
          warn
            (Loc.of_pos (Path.to_string path, i + 1, 0, String.length fn))
...
module Dir = struct
  ...
  let file_paths t = Path.Set.of_string_set (files t) ~f:(Path.relative t.path)
  ...
  let sub_dir_paths t =
    String.Map.foldi (sub_dirs t) ~init:Path.Set.empty ~f:(fun s _ acc ->
        Path.Set.add acc (Path.relative t.path s) )

  let rec fold t ~traverse_ignored_dirs ~init:acc ~f =
    if (not traverse_ignored_dirs) && t.ignored then acc
    else
      let acc = f t acc in
      ...
      lazy
        (let files, sub_dirs =
           ...
           |> List.filter_partition_map ~f:(fun fn ->
                  ...
                  if Path.is_in_build_dir path then Skip
                  else
                    let is_directory, file =
                      ...
                      | _ -> (false, File.dummy)
                    in
                    if ignore_file fn ~is_directory then Skip
                    else if is_directory then Right (fn, path, file)
                    else Left fn )
         in
         ...
         let project, dune_file, ignored_subdirs =
           if ignored then (project, None, String.Set.empty)
           else
             let project =
               Option.value
         ...
         let sub_dirs =
           List.fold_left sub_dirs ~init:String.Map.empty
             ~f:(fun acc (fn, path, file) ->
               let dirs_visited =
                 if Sys.win32 then dirs_visited
                 else
...
let rec find_dir t path =
  if not (Path.is_managed path) then None
  else
    ...
    | None -> (
      ...
      | Some dir as res ->
          ...
          res
      | None ->
          (* We don't cache failures in [t.dirs]. The expectation is that these
             only happen when the user writes an invalid path in a jbuild file,
             so there is no need to cache them. *)
          None )

let files_of t path =
  match find_dir t path with
  | None -> Path.Set.empty
  | Some dir -> Path.Set.of_string_set (Dir.files dir) ~f:(Path.relative path)
