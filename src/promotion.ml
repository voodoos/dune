...
module File = struct
  ...
  let _to_sexp {src; dst} =
    Sexp.List [Path.to_sexp src; Sexp.Atom "as"; Path.to_sexp dst]
...
let do_promote db files_to_promote =
  ...
  let potential_build_contexts =
    ...
    | files ->
        List.filter_map files ~f:(fun fn ->
            if fn = "" || fn.[0] = '.' || fn = "install" then None
            else
  ...
  | These (files, on_missing) ->
      let files = Path.Set.of_list files |> Path.Set.to_list in
      let by_targets =
...
let finalize () =
  let db =
    if !Clflags.auto_promote then do_promote !File.db All else !File.db
  in
  dump_db db
