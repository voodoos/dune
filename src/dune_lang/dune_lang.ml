...
let atom_or_quoted_string s =
  if Atom.is_valid_dune s then Atom (Atom.of_string s) else Quoted_string s
...
let rec to_string t ~syntax =
  ...
  | List l ->
      Printf.sprintf "(%s)"
        (List.map l ~f:(to_string ~syntax) |> String.concat ~sep:" ")
  | Template t -> Template.to_string t ~syntax

let rec pp syntax ppf = function
  | Atom s -> Format.pp_print_string ppf (Atom.print s syntax)
  | Quoted_string s -> Format.pp_print_string ppf (Escape.quoted ~syntax s)
...
let pp_quoted =
  let rec loop = function
    | Atom (A s) as t -> if Atom.is_valid_dune s then t else Quoted_string s
...
let pp_print_quoted_string ppf s =
  ...
  if String.contains s '\n' then (
    ...
    | first :: rest ->
        ...
        Format.fprintf ppf "@}\"@]" )
  else Format.pp_print_string ppf (Escape.quoted ~syntax s)
...
module Parser = struct
  let error (loc : Loc.t) message =
    raise (Parse_error {start = loc.start; stop = loc.stop; message})
  ...
  let parse ~mode ?(lexer = Lexer.token) lexbuf =
    loop 0 lexer lexbuf [] |> Mode.make_result mode lexbuf
end

let parse_string ~fname ~mode ?lexer str =
  let lb = Lexing.from_string str in
  lb.lex_curr_p <- {pos_fname = fname; pos_lnum = 1; pos_bol = 0; pos_cnum = 0};
...
module Encoder = struct
  ...
  let field name f ?(equal = ( = )) ?default v =
    match default with
    | None -> (name, Some (f v))
    | Some d -> if equal d v then (name, None) else (name, Some (f v))
...
module Decoder = struct
  ...
  let of_sexp_error ?hint loc msg = raise (Decoder (loc, msg, hint))
  ...
  let no_templates ?hint loc fmt =
    Printf.ksprintf
      (fun msg -> of_sexp_error loc ?hint ("No variables allowed " ^ msg))
      fmt

  type unparsed_field =
    { values : Ast.t list
    ; entry : Ast.t
    ; prev : unparsed_field option
    (* Previous occurrence of this field *) }
  ...
  let if_eos ~then_ ~else_ ctx state =
    if at_eos ctx state then then_ ctx state else else_ ctx state
  ...
  let result : type a k. k context -> a * k -> a =
   fun ctx (v, state) ->
    ...
    | Values (_, cstr, _) -> (
      ...
      | sexp :: _ -> (
        match cstr with
        | None -> of_sexp_errorf (Ast.loc sexp) "This value is unused"
        | Some s -> of_sexp_errorf (Ast.loc sexp) "Too many argument for %s" s
        ) )
    | Fields _ -> (
      ...
      | Some (name, {entry; _}) ->
          ...
          in
          of_sexp_errorf
            ~hint:{on = name; candidates = state.known}
  ...
  let[@inline never] end_of_list (Values (loc, cstr, _)) =
    match cstr with
    | None ->
        let loc = {loc with start = loc.stop} in
        of_sexp_errorf loc "Premature end of list"
    | Some s -> of_sexp_errorf loc "Not enough arguments for %s" s

  let[@inline always] next f ctx sexps =
    match sexps with [] -> end_of_list ctx | sexp :: sexps -> (f sexp, sexps)

  let[@inline always] next_with_user_context f ctx sexps =
    match sexps with
    | [] -> end_of_list ctx
    | sexp :: sexps -> (f (get_user_context ctx) sexp, sexps)

  let[@inline always] peek _ctx sexps =
    match sexps with [] -> (None, sexps) | sexp :: _ -> (Some sexp, sexps)

  let[@inline always] peek_exn ctx sexps =
  ...
  let enter t =
    next_with_user_context (fun uc sexp ->
        ...
        | List (loc, l) ->
            ...
            result ctx (t ctx l)
        | sexp -> of_sexp_error (Ast.loc sexp) "List expected" )
  ...
  let basic desc f =
    next (function
      ...
      | Atom (loc, s) -> (
        ...
        | Result.Error () -> of_sexp_errorf loc "%s expected" desc
        | Ok x -> x ) )
  ...
  let find_cstr cstrs loc name ctx values =
    ...
    | None ->
        of_sexp_errorf loc
          ~hint:{on = name; candidates = List.map cstrs ~f:fst}
  ...
  let sum cstrs =
    next_with_user_context (fun uc sexp ->
        ...
        | List (loc, []) -> of_sexp_error loc "Non-empty list expected"
        | List (loc, name :: args) -> (
          match name with
  ...
  let enum cstrs =
    next (function
      ...
      | Atom (loc, A s) -> (
        ...
        | None ->
            of_sexp_errorf loc
              ~hint:{on = s; candidates = List.map cstrs ~f:fst}
              "Unknown value %s" s ) )
  ...
  let[@inline never] field_missing loc name =
  ...
  let[@inline never] multiple_occurrences
      ?(on_dup = field_present_too_many_times) uc name last =
          ...
          | List (_, name_sexp :: values) -> (
            ...
            | Atom (_, A name) ->
                Name_map.add acc name
                  {values; entry = sexp; prev = Name_map.find acc name}
...
let rec to_sexp = function
  ...
  | Template t ->
      List [Atom "template"; Atom (Template.to_string ~syntax:Dune t)]
