...
struct
  ...
  module Dep = struct
    let html_alias m = Build_system.Alias.doc ~dir:(Paths.html m)
    ...
    let deps requires =
      Build.of_result_map requires ~f:(fun libs ->
          Build.path_set
            (List.fold_left libs ~init:Path.Set.empty
               ~f:(fun acc (lib : Lib.t) ->
                 if Lib.is_local lib then
                   let dir = Paths.odocs (Lib lib) in
                   Path.Set.add acc
                     (Build_system.Alias.stamp_file (alias ~dir))
                 else acc )) )
  ...
  let odoc =
    SC.resolve_program sctx "odoc" ~hint:"try: opam install odoc" ~loc:None
  ...
  let module_deps (m : Module.t) ~doc_dir ~(dep_graphs : Ocamldep.Dep_graphs.t)
      =
    ( if Module.has_intf m then Ocamldep.Dep_graph.deps_of dep_graphs.intf m
    else
  ...
  let setup_html (odoc_file : odoc) ~requires =
    ...
    SC.add_rule sctx
      ...
      >>> Build.progn
            ...
            :: Build.run ~dir:Paths.html_root odoc
                 [ A "html"
                 ; odoc_include_flags requires
                 ; A "-o"
                 ; Path Paths.html_root
  ...
  let setup_library_odoc_rules (library : Library.t) ~scope ~modules ~requires
      ~(dep_graphs : Ocamldep.Dep_graph.t Ml_kind.Dict.t) =
    let lib =
      Option.value_exn
    ...
    let modules_and_odoc_files =
      List.map
        ...
        ~f:(compile_module ~obj_dir ~includes ~dep_graphs ~doc_dir ~pkg_or_lnu)
    in
    Dep.setup_deps (Lib lib)
      (List.map modules_and_odoc_files ~f:snd |> Path.Set.of_list)

  let setup_css_rule () =
    SC.add_rule sctx
      (Build.run ~dir:context.build_dir odoc
         [A "css"; A "-o"; Path Paths.html_root; Hidden_targets [css_file]])
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  let create_odoc ~target odoc_input =
    ...
    | Lib _ ->
        let html_dir =
          html_base
          ++ ( Path.basename odoc_input |> Filename.chop_extension
  ...
  let odocs =
    let odoc_glob =
      Re.compile (Re.seq [Re.(rep1 any); Re.str ".odoc"; Re.eos])
    in
    fun target ->
  ...
  let setup_lib_html_rules =
    ...
    fun lib ~requires ->
      if not (Lib.Set.mem !loaded lib) then (
  ...
  let setup_pkg_html_rules =
    ...
    fun ~pkg ~libs ->
      if not (Package.Name.Table.get loaded pkg) then (
        ...
        let odocs =
          List.concat
            (pkg_odocs :: List.map libs ~f:(fun lib -> odocs (Lib lib)))
        in
        let html_files = List.map ~f:(fun o -> o.html_file) odocs in
    ...
    | "_html" :: lib_unique_name_or_pkg :: _ ->
        ...
        let setup_pkg_html_rules pkg =
          setup_pkg_html_rules ~pkg
            ~libs:(Lib.Set.to_list (load_all_odoc_rules_pkg ~pkg))
        in
        ( match Lib.DB.find lib_db lib with
        | Error _ -> ()
        | Ok lib -> (
          match Lib.package lib with
          | None ->
              setup_lib_html_rules lib
                ~requires:(Lib.closure ~linking:false [lib])
  ...
  let setup_package_aliases (pkg : Package.t) =
    let alias =
      Build_system.Alias.doc
        ~dir:(Path.append context.build_dir pkg.Package.path)
    in
    SC.add_alias_deps sctx alias
      ( Dep.html_alias (Pkg pkg.name)
        :: ( libs_of_pkg ~pkg:pkg.name |> Lib.Set.to_list
  ...
  let entry_modules ~(pkg : Package.t) =
    ...
    |> List.filter_map ~f:(fun l ->
           if Lib.is_local l then Some (l, entry_modules_by_lib l) else None )
    |> Lib.Map.of_list_exn
  ...
  let setup_package_odoc_rules ~pkg ~mlds =
    ...
    let mlds =
      if String.Map.mem mlds "index" then mlds
        ...
        String.Map.add mlds "index" gen_mld
    in
    let odocs =
      List.map (String.Map.values mlds) ~f:(fun mld ->
          compile_mld (Mld.create mld) ~pkg:pkg.name
  ...
  let init () =
    let mlds_by_package =
      ...
      fun (p : Package.t) ->
        Option.value
          (Package.Name.Map.find (Lazy.force map) p.name)
          ~default:[]
    in
    SC.packages sctx
    |> Package.Name.Map.iter ~f:(fun (pkg : Package.t) ->
           let rules =
             lazy (setup_package_odoc_rules ~pkg ~mlds:(mlds_by_package pkg))
           in
           List.iter [Paths.odocs (Pkg pkg.name); Paths.gen_mld_dir pkg]
             ~f:(fun dir ->
