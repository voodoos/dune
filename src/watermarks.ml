...
let is_a_source_file fn =
  ...
  | ".flv" | ".gif" | ".ico" | ".jpeg" | ".jpg" | ".mov" | ".mp3" | ".mp4"
   |".otf" | ".pdf" | ".png" | ".ttf" | ".woff" ->
      false
  | _ -> true
  ...
  let name =
    ...
    | Dune -> (
        ...
        if not (List.mem ~set:files Dune_project.filename) then
          die
            "@{<error>Error@}: There is no dune-project file in the current \
        ...
        | None ->
            die
              "@{<error>Error@}: The project name is not defined, please add \
        ...
        | None -> (
            let name =
              ...
              if prefix = "" then None
                ...
                match
                  ( String.drop_suffix prefix ~suffix:"-"
                  , String.drop_suffix prefix ~suffix:"_" )
                with
            ...
            | None ->
                die
                  "@{<error>Error@}: cannot determine name automatically.\n\
...
let subst_git ?name () =
  ...
  Fiber.fork_and_join
    (fun () ->
      Fiber.fork_and_join
        (fun () ->
          Process.run_capture Strict git
            ["describe"; "--always"; "--dirty"]
            ~env )
        (fun () -> Process.run_capture Strict git ["rev-parse"; rev] ~env) )
    (fun () ->
...
let subst ?name () =
  if Sys.file_exists ".git" then subst_git ?name () else Fiber.return ()
