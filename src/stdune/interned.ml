...
module Make (R : Settings) () = struct
  ...
  module Table = struct
    ...
    let get t key =
      if key >= Array.length t.data then t.default_value else t.data.(key)
...
module No_interning (R : Settings) () = struct
  ...
  module Table = struct
    ...
    let create ~default_value =
      {default_value; data = Hashtbl.create R.initial_size}

    let get t k =
      match Hashtbl.find t.data k with None -> t.default_value | Some s -> s
