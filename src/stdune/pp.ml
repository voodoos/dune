...
module Renderer = struct
  ...
  module Make (Tag : Tag) = struct
    ...
    let embed_tag ~opening ~closing =
      ...
      Bytes.blit_string ~src:opening ~src_pos:0 ~dst:buf ~dst_pos:2
        ~len:opening_len;
      Bytes.blit_string ~src:closing ~src_pos:0 ~dst:buf
        ~dst_pos:(2 + opening_len) ~len:closing_len;
