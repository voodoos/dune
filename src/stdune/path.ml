...
module Local : sig
  ...
  let equal x y = match compare x y with Eq -> true | Gt | Lt -> false
  ...
  let parent t =
    if is_root t then Exn.code_error "Path.Local.parent called on the root" []
    else
      let t = to_string t in
  ...
  let relative ?error_loc t path =
    ...
    let rec loop t components =
      ...
      | "." :: rest -> loop t rest
      | ".." :: rest ->
          if is_root t then Result.Error () else loop (parent t) rest
      | fn :: rest ->
          if is_root t then loop (make fn) rest
          else loop (make (to_string t ^ "/" ^ fn)) rest
    in
  ...
  let is_canonicalized =
    ...
    and before_dot_dot_slash s i =
      if i < 0 then false
      else match s.[i] with '/' -> false | _ -> in_component s (i - 1)
  ...
  let descendant t ~of_ =
    if is_root of_ then Some t
      ...
      then Some (make (String.drop t (of_len + 1)))
      else None
  ...
  module Prefix = struct
    ...
    let make p =
      if is_root p then
        Exn.code_error "Path.Local.Prefix.make" [("path", to_sexp p)];
...
module Kind = struct
  ...
  let of_string s =
    if Filename.is_relative s then Local (Local.of_string s)
...
let of_string ?error_loc s =
  ...
  | s ->
      if not (Filename.is_relative s) then external_ (External.of_string s)
      else make_local_path (Local.of_string s ?error_loc)
...
let parent = function
  | External s ->
      ...
      if parent = s then None else Some (external_ parent)
...
let extract_build_context = function
  ...
  | In_build_dir t -> (
      ...
      | None -> Some (t, in_source_tree Local.root)
      | Some (before, after) ->
  ...
  | In_build_dir t -> (
      ...
      | Some (before, after) ->
          Some
            ...
            , after |> Local.of_string |> in_source_tree ) )
...
let drop_optional_build_context t =
  match extract_build_context t with None -> t | Some (_, t) -> t
...
let win32_unlink fn =
  try Unix.unlink fn with Unix.Unix_error (Unix.EACCES, _, _) as e -> (
    try
...
module Set = struct
  ...
  let of_string_set ss ~f = String.Set.to_list ss |> List.map ~f |> of_list
end
