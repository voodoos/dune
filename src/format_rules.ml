...
let flag_of_kind : Ml_kind.t -> _ = function
  | Impl -> "--impl"
  | Intf -> "--intf"

let config_includes (config : Dune_file.Auto_format.t) s =
  match config.enabled_for with Default -> true | Only set -> List.mem s ~set
...
let add_diff sctx loc alias ~dir input output =
  ...
  SC.add_alias_action sctx alias ~loc:(Some loc) ~locks:[] ~stamp:input
    (Build.paths [input; output] >>> Build.action ~dir ~targets:[] action)
...
let gen_rules sctx (config : Dune_file.Auto_format.t) ~dir =
  ...
  let setup_formatting file (arrows_acc, extra_deps_acc) =
    ...
    let formatter =
      ...
      | ".ml" -> ocaml Impl
      | ".mli" -> ocaml Intf
      | (".re" | ".rei") when config_includes config Reason ->
  ...
  Super_context.on_load_dir sctx ~dir:output_dir ~f:(fun () ->
      ...
      in
      List.iter arrows ~f:(fun (format_arr, input, output) ->
          let open Build.O in
