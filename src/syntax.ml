...
module Version = struct
  ...
  let decode : t Dune_lang.Decoder.t =
    ...
    >>| function
    | Atom (loc, A s) -> (
      try Scanf.sscanf s "%u.%u" (fun a b -> (a, b)) with _ ->
        Errors.fail loc "Atom of the form NNN.NNN expected" )
    | sexp -> of_sexp_error (Dune_lang.Ast.loc sexp) "Atom expected"
...
module Supported_versions = struct
  ...
  let make l : t =
    match
      List.map l ~f:(fun (major, minor) -> (major, minor)) |> Int.Map.of_list
    with
    | Ok x -> x
    | Error _ ->
        Exn.code_error "Syntax.create"
...
module Error = struct
  ...
  let renamed_in loc t ver ~what ~to_ =
    Errors.fail loc "%s was renamed to '%s' in the %s version of %s" what to_
      (Version.to_string ver) t.desc

  let deleted_in loc t ?repl ver ~what =
    Errors.fail loc "%s was deleted in version %s of %s%s" what
      (Version.to_string ver) t.desc
      (match repl with None -> "" | Some s -> ".\n" ^ s)
end
...
let get_exn t =
  ...
  | None ->
      ...
      Exn.code_error "Syntax identifier is unset"
        ...
        ; ( "supported_versions"
          , Supported_versions.to_sexp t.supported_versions )
...
let deleted_in t ver =
  ...
  else desc () >>= fun (loc, what) -> Error.deleted_in loc t ver ~what
...
let renamed_in t ver ~to_ =
  ...
  >>= fun current_ver ->
  if current_ver < ver then return ()
  else desc () >>= fun (loc, what) -> Error.renamed_in loc t ver ~what ~to_
