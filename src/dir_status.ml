...
type t =
  ...
  (* Directory not part of a multi-directory group. The argument is
     ...
     generated ones. *)
  | Group_root of File_tree.Dir.t * Super_context.Dir_with_dune.t
...
let is_standalone = function Standalone _ -> true | _ -> false
  ...
  | None ->
      let t =
        ...
        | None -> (
          ...
          | Some dir ->
              if is_standalone (get sctx ~dir) then Standalone None
              else Is_component_of_a_group_but_not_the_root None )
        | Some ft_dir -> (
            let project_root =
              File_tree.Dir.project ft_dir
              |> Dune_project.root |> Path.of_local
            in
            match Super_context.stanzas_in sctx ~dir with
            | None ->
                if
                  Path.equal dir project_root
                  || is_standalone (get sctx ~dir:(Path.parent_exn dir))
                then Standalone (Some (ft_dir, None))
                else Is_component_of_a_group_but_not_the_root None
            | Some d -> (
              match get_include_subdirs d.stanzas with
              | Some Unqualified -> Group_root (ft_dir, d)
              ...
              | None ->
                  ...
                  else Standalone (Some (ft_dir, Some d)) ) )
      in
