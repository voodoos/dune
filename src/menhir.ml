...
module Run (P : PARAMS) : sig end = struct
  ...
  let mock_ml m : Path.t = Path.relative dir (mock m ^ ".ml.mock")

  let inferred_mli m : Path.t = Path.relative dir (mock m ^ ".mli.inferred")
  ...
  let expand_flags flags =
    Super_context.expand_and_eval_set sctx
      ...
      ~dir ~standard:(Build.return []) flags
  ...
  let process3 base ~cmly (stanza : stanza) : unit =
    ...
    Module_compilation.ocamlc_i
      ...
      cctx mock_module ~output:(inferred_mli base);
