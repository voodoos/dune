...
module Linkage = struct
  ...
  let of_user_config (ctx : Context.t) (m : Dune_file.Executables.Link_mode.t)
    ...
    let ext =
      ...
      | Byte, C -> ".bc.c"
      | Native, C ->
          Errors.fail m.loc "C file generation only supports bytecode!"
      | Byte, Exe -> ".bc"
      | Native, Exe -> ".exe"
    ...
    let flags =
      ...
      | Shared_object -> (
          let so_flags =
            if String.equal ctx.os_type "Win32" then so_flags_windows
            else so_flags_unix
          in
...
let link_exe ~loc ~name ~(linkage : Linkage.t) ~top_sorted_modules
    ~arg_spec_for_requires ?(link_flags = Build.arr (fun _ -> []))
    ?(js_of_ocaml = Dune_file.Js_of_ocaml.default) cctx =
  ...
  let modules_and_cm_files =
    Build.memoize "cm files"
      ( top_sorted_modules
      >>^ fun modules ->
      (modules, artifacts modules ~ext:(Cm_kind.ext (Mode.cm_kind mode))) )
  in
  ...
  SC.add_rule sctx ~loc
    ...
    >>> Build.run ~dir:ctx.build_dir (Ok compiler)
...
let build_and_link_many ~programs ~linkages ?link_flags
    ?(js_of_ocaml = Dune_file.Js_of_ocaml.default) cctx =
  ...
  List.iter programs ~f:(fun {Program.name; main_module_name; loc} ->
      let top_sorted_modules =
        let main =
          Option.value_exn
            (Module.Name.Map.find (CC.modules cctx) main_module_name)
        in
        Ocamldep.Dep_graph.top_closed_implementations dep_graphs.impl [main]
      in
      let arg_spec_for_requires =
        Mode.Dict.of_func (fun ~mode ->
            lazy
              (Result.map (CC.requires cctx)
                 ~f:(Link_time_code_gen.libraries_link ~loc ~name ~mode cctx))
        )
      in
      List.iter linkages ~f:(fun linkage ->
          link_exe cctx ~loc ~name ~linkage ~top_sorted_modules ~js_of_ocaml
            ~arg_spec_for_requires ?link_flags ) )
