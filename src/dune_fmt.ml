...
let parse_file path_opt =
  let fname, contents =
    ...
    | None ->
        ...
        ("<stdin>", contents)
  in
  Dune_lang.parse_string ~fname ~mode:Dune_lang.Parser.Mode.Many contents

let can_be_displayed_inline =
  List.for_all ~f:(function
    | Dune_lang.Atom _ | Dune_lang.Quoted_string _ | Dune_lang.Template _
     |Dune_lang.List [_] ->
        true
    | Dune_lang.List _ -> false )

let pp_indent fmt indent = Format.pp_print_string fmt @@ String.make indent ' '
...
let rec pp_sexp indent fmt = function
  | (Dune_lang.Atom _ | Dune_lang.Quoted_string _ | Dune_lang.Template _) as
    sexp ->
      Format.fprintf fmt "%a%a" pp_indent indent
        (Dune_lang.pp Dune_lang.Dune)
        sexp
  | Dune_lang.List sexps ->
      if can_be_displayed_inline sexps then print_inline_list fmt indent sexps
      else pp_sexp_list indent fmt sexps
