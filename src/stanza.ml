...
module Decoder = struct
  ...
  let () =
    Report_error.register (function
      | Parens_no_longer_necessary (loc, exn) ->
          let hint =
            "dune files require less parentheses than jbuild files.\n\
             If you just converted this file from a jbuild file, try removing \
             these parentheses."
          in
          Option.map (Report_error.find_printer exn) ~f:(fun printer ->
              printer |> Report_error.set_loc ~loc
              |> Report_error.set_hint ~hint )
      | _ -> None )

  let switch_file_kind ~jbuild ~dune =
    file_kind () >>= function Jbuild -> jbuild | Dune -> dune

  let parens_removed_in_dune_generic ~is_record t =
    switch_file_kind ~jbuild:(enter t)
      ~dune:
        (try_ t (function
          ...
          | exn ->
              try_
                (enter
                   ...
                   >>= fun _ -> raise (Parens_no_longer_necessary (loc, exn))
                   ))
                (function
                  | Parens_no_longer_necessary _ as exn -> raise exn
                  | _ -> raise exn) ))
  ...
  let list parse = parens_removed_in_dune (repeat parse)
  ...
  let on_dup parsing_context name entries =
    ...
    | Some (0, _) ->
        ...
        Errors.warn (Dune_lang.Ast.loc last)
          ...
          name
    | _ -> field_present_too_many_times parsing_context name entries
