...
module Local = struct
  ...
  let of_string (name : string) =
    ...
    | (s : string) ->
        if s.[0] = '.' then Invalid
          ...
          let rec loop warn i =
            if i = len - 1 then if warn then Warn s else Ok s
            else
  ...
  let of_string_exn s =
    ...
    | Ok s -> s
    | Warn _ | Invalid -> raise (Invalid_lib_name s)
  ...
  let wrapped_message =
    sprintf
      "%s.\n\
       ...
       It will not be supported in the future. Please choose a valid name \
       field."
      invalid_message
  ...
  let validate (loc, res) ~wrapped =
    ...
    | Warn s, false ->
        Errors.warn loc "%s" wrapped_message;
        s
