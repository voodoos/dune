...
let build_cm cctx ?sandbox ?(dynlink = true) ~dep_graphs ~cm_kind
  ...
  Mode.of_cm_kind cm_kind |> Context.compiler ctx
  |> Option.iter ~f:(fun compiler ->
         Option.iter (Module.cm_source m cm_kind) ~f:(fun src ->
             ...
             let other_cm_files =
               Build.dyn_paths
                 ( Ocamldep.Dep_graph.deps_of dep_graph m
                 >>^ fun deps ->
             ...
             in
             let hidden_targets =
               List.map other_targets ~f:(Target.file obj_dir)
             in
             ...
             SC.add_rule sctx ?sandbox
               ( Build.paths extra_deps >>> other_cm_files >>> flags
               >>> Build.run ~dir (Ok compiler)
                     ...
                     ; ( match private_obj_dir with
                       | None -> S []
                       | Some private_obj_dir ->
                           S [A "-I"; Path private_obj_dir] )
                     ; Cm_kind.Dict.get (CC.includes cctx) cm_kind
                     ; As extra_args
                     ; ( if dynlink || cm_kind <> Cmx then As []
                       else A "-nodynlink" )
                     ; A "-no-alias-deps"
                     ; opaque_arg
                     ...
                     ; As
                         ...
                         | None -> []
                         | Some
                             {Dune_file.Library.Stdlib.modules_before_stdlib; _}
                           -> (
                             let flags = ["-nopervasives"; "-nostdlib"] in
                             if
                               Module.Name.Set.mem modules_before_stdlib
                                 (Module.name m)
                             then flags
                             else
...
let ocamlc_i ?sandbox ?(flags = []) ~dep_graphs cctx (m : Module.t) ~output =
  ...
  let cm_deps =
    Build.dyn_paths
      ( Ocamldep.Dep_graph.deps_of dep_graph m
      >>^ fun deps ->
      List.concat_map deps ~f:(fun m -> [Module.cm_file_unsafe m ~obj_dir Cmi])
      )
  in
  SC.add_rule sctx ?sandbox
    ( cm_deps
