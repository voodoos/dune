...
(** Overlay for [Dune_lang.Decoder] where lists and records don't require an
    extra level of parentheses in Dune files.

    Additionally, [field_xxx] functions only warn about duplicated fields in
    jbuild files, for backward compatibility. *)
module Decoder : sig
  ...
  (** Nop in dune files and [enter t] in jbuild files. Additionally it displays
      a nice error messages when parentheses are used in dune files. *)
  val parens_removed_in_dune : 'a t -> 'a t

  (** Use a different parser depending on the syntax in the current file. If
      the syntax version is strictly less than `(1, 0)`, use `jbuild`.
      Otherwise use `dune`. *)
