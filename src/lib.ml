...
module Error = struct
  ...
  module No_implementation = struct
    type t = {for_vlib : Lib_info.t * Dep_path.Entry.t list}
  end
...
let to_id t : Id.t =
  {unique_id = t.unique_id; path = t.info.src_dir; name = t.name}
...
module L = struct
  ...
  let include_flags ts ~stdlib_dir = to_iflags (include_paths ts ~stdlib_dir)
  ...
  let remove_dups l =
    let rec loop acc l seen =
      ...
      | [] -> acc
      | x :: l ->
          if Int.Set.mem seen x.unique_id then loop acc l seen
          else loop (x :: acc) l (Int.Set.add seen x.unique_id)
    in
  ...
  let link_flags ts ~mode ~stdlib_dir =
    ...
    Arg_spec.S
      ...
      :: List.map ts ~f:(function
           | Lib t -> Arg_spec.Deps (Mode.Dict.get t.info.archives mode)
           | Module (m, obj_dir) ->
    ...
    | M.Info.T info ->
        let get ~loc lib' =
          if lib.unique_id = lib'.unique_id then
            Errors.fail loc "Library %a depends on itself" Lib_name.pp_quoted
              lib.name
          else M.get lib'
        in
...
module Dep_stack = struct
  ...
  let empty = {stack = []; seen = Int.Set.empty}

  let to_required_by t ~stop_at =
    let stop_at = stop_at.stack in
    let rec loop acc l =
      if List.physically_equal l stop_at then List.rev acc
      else
        match l with
        | [] -> assert false
        | {Id.path; name; _} :: l ->
  ...
  let dependency_cycle t (last : Id.t) =
    ...
    let rec build_loop acc stack =
      ...
      | (x : Id.t) :: stack ->
          ...
          if x.unique_id = last.unique_id then acc else build_loop acc stack
    in
  ...
  let create_and_push t name path =
    ...
    (init, {stack = init :: t.stack; seen = Int.Set.add t.seen unique_id})

  let push t (x : Id.t) : (_, _) result =
    if Int.Set.mem t.seen x.unique_id then Error (dependency_cycle t x)
    else Ok {stack = x :: t.stack; seen = Int.Set.add t.seen x.unique_id}
end
...
let check_private_deps lib ~loc ~allow_private_deps =
  ...
  then
    Result.Error
      (Error (Private_deps_not_allowed {private_dep = lib.info; loc}))
  else Ok lib
...
let already_in_table (info : Lib_info.t) name x =
  ...
  let sexp =
    match x with
    | St_initializing x ->
        Sexp.List [Sexp.Atom "Initializing"; Path.to_sexp x.path]
    | St_found t -> List [Sexp.Atom "Found"; Path.to_sexp t.info.src_dir]
    | St_not_found -> Sexp.Atom "Not_found"
    | St_hidden (_, {path; reason; _}) ->
        List [Sexp.Atom "Hidden"; Path.to_sexp path; Sexp.Atom reason]
  in
  Exn.code_error
...
module Virtual_libs : sig
      ...
      let make closure ~orig_stack : t Or_exn.t =
        let rec loop acc = function
          ...
          | (lib, stack) :: libs -> (
            ...
            | None, Some _ -> loop (Map.add acc lib (No_impl stack)) libs
            | Some vlib, None -> (
                vlib
                >>= fun vlib ->
    ...
    let make impls ~orig_stack : t Or_exn.t =
      let rec loop acc = function
        ...
        | (vlib, Partial.No_impl stack) :: _ ->
            ...
            Error (Error (No_implementation {for_vlib = (vlib.info, rb)}))
  ...
  let second_step_closure ts impls =
    ...
    let rec loop t =
      ...
      if Int.Set.mem !visited t.unique_id then Ok ()
      else (
        visited := Int.Set.add !visited t.unique_id;
        t.requires
        >>= fun deps ->
        Result.List.iter deps ~f:loop >>| fun () -> res := t :: !res )
    in
    Result.List.iter ts ~f:loop >>| fun () -> List.rev !res
  ...
  let associate closure ~orig_stack ~linking =
    ...
    if linking && not (Table.Partial.is_empty impls) then
      ...
      >>= fun impls -> second_step_closure closure impls
    else Ok closure
end

let rec instantiate db name (info : Lib_info.t) ~stack ~hidden =
  let id, stack = Dep_stack.create_and_push stack name info.src_dir in
  ...
  let requires =
    ...
    | None -> requires
    | Some impl ->
        impl >>= fun impl -> requires >>| fun requires -> impl :: requires
  in
  ...
  let res =
    ...
    match hidden with
    | None -> St_found t
    | Some reason -> St_hidden (t, {name; path = t.info.src_dir; reason})
  in
...
and resolve_dep db (name : Lib_name.t) ~allow_private_deps ~loc ~stack :
    t Or_exn.t =
  match find_internal db name ~stack with
  | St_initializing id -> Error (Dep_stack.dependency_cycle stack id)
  ...
  let res, resolved_selects =
    List.fold_left deps ~init:(Ok [], []) ~f:(fun (acc_res, acc_selects) dep ->
          ...
          | Select {result_fn; choices; loc} ->
              let res, src_fn =
                ...
                | None ->
                    ...
                    (Error (Error (No_solution_found_for_select e)), Error e)
              in
        ...
        let res =
          ...
          | (Error _ as res), _ | _, (Error _ as res) -> res
        in
        (res, acc_selects) )
  in
...
and resolve_deps db deps ~allow_private_deps ~stack =
  ...
  | Complex names -> resolve_complex_deps ~allow_private_deps db names ~stack
...
and closure_with_overlap_checks db ts ~stack ~linking =
  ...
  let rec loop t ~stack =
    ...
    | Some (t', stack') ->
        if t.unique_id = t'.unique_id then Ok ()
        else
          let req_by = Dep_stack.to_required_by ~stop_at:orig_stack in
          Error
            (Error
               (Conflict
                  { lib1 = (t'.info, req_by stack')
          ...
          | St_found t' ->
              if t.unique_id = t'.unique_id then Ok ()
                ...
                Error
                  (Error
                     (Overlap
                        {in_workspace = t'.info; installed = (t.info, req_by)}))
          | _ -> assert false ) )
        >>= fun () ->
        Dep_stack.push stack (to_id t)
        >>= fun new_stack ->
        t.requires
        >>= fun deps ->
        Result.List.iter deps ~f:(loop ~stack:new_stack)
        >>| fun () -> res := (t, stack) :: !res
  in
  Result.List.iter ts ~f:(loop ~stack)
  >>= fun () -> Virtual_libs.associate (List.rev !res) ~linking ~orig_stack
...
let to_exn res = match res with Ok x -> x | Error e -> raise e
...
module Compile = struct
  ...
  let for_lib db (t : lib) =
    ...
    ; requires = t.requires >>= closure_with_overlap_checks db ~linking:false
...
module DB = struct
  ...
  let create ?parent ~resolve ~all () =
    {parent; resolve; table = Hashtbl.create 1024; all = Lazy.from_fun all}
  ...
  let create_from_library_stanzas ?parent ~ext_lib ~ext_obj stanzas =
    let map =
      List.concat_map stanzas ~f:(fun (dir, (conf : Dune_file.Library.t)) ->
          ...
          | Some p ->
              ...
              if name = Lib_name.of_local conf.name then [(name, Found info)]
              else
                [ (name, Found info)
      ...
      | Error (name, _, _) -> (
        match
          List.filter_map stanzas ~f:(fun (_, (conf : Dune_file.Library.t)) ->
              if
                name = Lib_name.of_local conf.name
                ||
                match conf.public with
                | None -> false
    ...
    create () ?parent
      ~resolve:(fun name ->
        match Lib_name.Map.find map name with None -> Not_found | Some x -> x
        )
    ...
    create ()
      ~resolve:(fun name ->
        ...
        | Error e -> (
          ...
          | Hidden pkg ->
              Hidden (Lib_info.of_findlib_package pkg, "unsatisfied 'exist_if'")
          ) )
      ~all:(fun () ->
        Findlib.all_packages findlib |> List.map ~f:Findlib.Package.name )
  ...
  let resolve t (loc, name) =
    ...
    | Ok _ as res -> res
    | Error reason -> Error (Error (Library_not_available {loc; name; reason}))
  ...
  let rec all ?(recursive = false) t =
    let l =
      List.fold_left (Lazy.force t.all)
        ~f:(fun libs name ->
          match find t name with Ok x -> Set.add libs x | Error _ -> libs )
        ~init:Set.empty
    in
...
let report_lib_error ppf (e : Error.t) =
  let dep_path ppf dp =
    match dp with
    | [] -> ()
    | _ -> Format.fprintf ppf "@,   %a" Dep_path.Entries.pp dp
  in
  let lib ppf (info : Lib_info.t) =
    Format.fprintf ppf "%a in %a" Lib_name.pp_quoted info.name Path.pp
      info.src_dir
  in
  ...
  | Double_implementation {impl1; impl2; vlib} ->
      Format.fprintf ppf
        "@[<v>@{<error>Error@}: Conflicting implementations for virtual \
         library %a:@,- %a@,- %a@,This cannot work.@]"
        Lib_name.pp_quoted vlib lib_and_dep_path impl1 lib_and_dep_path impl2
  | No_implementation {for_vlib = info, dp} ->
      Format.fprintf ppf
        "@[<v 3>@{<error>Error@}: No implementation found for virtual library \
         %a (%a).%a@]"
        Lib_name.pp_quoted info.name Path.pp info.src_dir dep_path dp
  | Library_not_available {loc = _; name; reason} ->
      Format.fprintf ppf "@{<error>Error@}: Library %a %a.@\n"
        Lib_name.pp_quoted name Error.Library_not_available.Reason.pp reason
  | Conflict {lib1; lib2} ->
      Format.fprintf ppf
        "@[<v>@{<error>Error@}: Conflict between the following libraries:@,- \
         %a@,- %a@,This cannot work.@]@\n"
        lib_and_dep_path lib1 lib_and_dep_path lib2
  | Overlap {in_workspace; installed} ->
      Format.fprintf ppf
        "@[<v>@{<error>Error@}: Conflict between the following libraries:@,- \
         %a@,- %a@,This is not allowed.@]@\n"
        lib in_workspace lib_and_dep_path installed
  ...
  | Dependency_cycle cycle ->
      Format.fprintf ppf
        "@{<error>Error@}: Dependency cycle detected between the following \
         libraries:@\n\
         @[<v>%a@]@\n"
        (Format.pp_print_list (fun ppf (path, name) ->
             Format.fprintf ppf "-> %a in %a" Lib_name.pp_quoted name Path.pp
               path ))
        cycle
  | Private_deps_not_allowed t ->
      Format.fprintf ppf
        "@{<error>Error@}: Library %a is private, it cannot be a dependency \
         of a public library.\n\
         You need to give %a a public name.\n"
        Lib_name.pp_quoted t.private_dep.name Lib_name.pp_quoted
        t.private_dep.name
            ...
            | Library_not_available {loc; _} -> (
                ...
                , match !Clflags.external_lib_deps_hint with
                  ...
                  | l ->
                      ...
                      in
                      Some ("try: " ^ cmdline) ) )
            | Private_deps_not_allowed t -> (Some t.loc, None)
            | _ -> (None, None)
          in
