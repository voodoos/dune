...
let user_rule sctx ?extra_bindings ~dir ~scope (rule : Rule.t) =
  if SC.eval_blang sctx rule.enabled_if ~dir ~scope then
    ...
    SC.add_rule_get_targets sctx ~mode:rule.mode ~loc:rule.loc
      ...
      >>> SC.Action.run sctx (snd rule.action) ~loc:(fst rule.action) ~dir
            ~bindings ~dep_kind:Required ~targets ~targets_dir:dir ~scope )
  else []
...
let copy_files sctx ~dir ~scope ~src_dir (def : Copy_files.t) =
  ...
  if
    ...
    && not (Path.is_descendant glob_in_src ~of_:src_dir)
  then
  ...
  if not (File_tree.dir_exists file_tree src_in_src) then
    Errors.fail loc "cannot find directory: %a" Path.pp src_in_src;
  (* add rules *)
  ...
  List.map files ~f:(fun basename ->
      ...
      SC.add_rule sctx ~loc
        (( if def.add_line_directive then Build.copy_and_add_line_directive
         else Build.copy )
...
let alias sctx ?extra_bindings ~dir ~scope (alias_conf : Alias_conf.t) =
  let stamp =
    ( "user-alias"
    , Dune_file.Bindings.map ~f:Dune_file.Dep_conf.remove_locs alias_conf.deps
    , Option.map
        ~f:(fun (_loc, a) -> Action.Unexpanded.remove_locs a)
        alias_conf.action
  ...
  if SC.eval_blang sctx alias_conf.enabled_if ~dir ~scope then
    add_alias sctx ~dir ~loc ~name:alias_conf.name ~stamp
      ...
      | Some (loc, action) ->
          ...
          SC.Action.run sctx action ~loc ~dir ~dep_kind:Required ~bindings
            ~targets:Alias ~targets_dir:dir ~scope )
  else
