...
let map_loc (loc : Loc.t) : Loc.t =
  {start = map_pos loc.start; stop = map_pos loc.stop}
...
let set_loc p ~loc = {p with loc = Some loc}

let set_hint p ~hint = {p with hint = Some hint}

let builtin_printer = function
  | Dune_lang.Decoder.Decoder (loc, msg, hint') ->
      let pp ppf =
        Format.fprintf ppf "@{<error>Error@}: %s%s\n" msg
          ( match hint' with
          | None -> ""
          | Some {Dune_lang.Decoder.on; candidates} -> hint on candidates )
      in
...
let find_printer exn = List.find_map !printers ~f:(fun f -> f exn)

let exn_printer exn =
  let pp ppf =
    let s = Printexc.to_string exn in
    if String.is_prefix s ~prefix:"File \"" then Format.fprintf ppf "%s\n" s
    else Format.fprintf ppf "@{<error>Error@}: exception %s\n" s
  in
  make_printer ~backtrace:true pp

(* Firt return value is [true] if the backtrace was printed *)
let report_with_backtrace exn =
  match find_printer exn with Some p -> p | None -> exn_printer exn

let reported = ref String.Set.empty

let clear_cache () = reported := String.Set.empty
...
let report exn =
  ...
  | _ ->
      ...
      let loc =
        if Option.equal Loc.equal p.loc (Some Loc.none) then None else p.loc
      in
      ...
      if String.Set.mem !reported hash then Buffer.clear err_buf
      else (
        ...
        let dependency_path =
          ...
          if !Clflags.debug_dep_path then dependency_path
          else
