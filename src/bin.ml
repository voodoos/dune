...
let path =
  match Env.get Env.initial "PATH" with None -> [] | Some s -> parse_path s

let exe = if Sys.win32 then ".exe" else ""

let best_prog dir prog =
  let fn = Path.relative dir (prog ^ ".opt" ^ exe) in
  if Path.exists fn then Some fn
  else
    let fn = Path.relative dir (prog ^ exe) in
    if Path.exists fn then Some fn else None
...
let make = match which "gmake" with None -> which "make" | some -> some
