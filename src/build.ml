  ...
  let get_glob_result_exn state =
    ...
    | G_unevaluated (loc, path, _) ->
        Exn.code_error "Build.get_glob_result_exn: got unevaluated"
          [("loc", Loc.to_sexp loc); ("path", Path.to_sexp path)]
end
...
let paths_matching ~loc ~dir f = Paths_glob (ref (G_unevaluated (loc, dir, f)))
...
let file_exists p =
  if_file_exists p ~then_:(arr (fun _ -> true)) ~else_:(arr (fun _ -> false))

let file_exists_opt p t =
  if_file_exists p ~then_:(t >>^ fun x -> Some x) ~else_:(arr (fun _ -> None))

let fail ?targets x =
  match targets with None -> Fail x | Some l -> Targets l >>> Fail x
...
let write_file_dyn fn = Targets [fn] >>^ fun s -> Action.Write_file (fn, s)
...
let symlink ~src ~dst = path src >>> action ~targets:[dst] (Symlink (src, dst))

let create_file fn = action ~targets:[fn] (Redirect (Stdout, fn, Progn []))

let remove_tree dir = arr (fun _ -> Action.Remove_tree dir)

let mkdir dir = arr (fun _ -> Action.Mkdir dir)
