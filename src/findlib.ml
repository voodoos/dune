...
module Config = struct
  ...
  let pp fmt {vars; preds} =
    Fmt.record fmt
      [ ( "vars"
        , Fmt.const
            (Fmt.ocaml_list (Fmt.tuple Format.pp_print_string Rules.pp))
            (String.Map.to_list vars) )
      ; ("preds", Fmt.const Ps.pp preds) ]
  ...
  let load path ~toolchain ~context =
    ...
    if not (Path.exists conf_file) then
      die
        "@{<error>Error@}: ocamlfind toolchain %s isn't defined in %a \
         (context: %s)"
        toolchain Path.pp path context;
...
let parse_package t ~meta_file ~name ~parent_dir ~vars =
  ...
  in
  let pkg = {Package.meta_file; name; dir; vars} in
  ...
  let rec loop dirs : (Path.t * Path.t * Meta.Simplified.t) option =
    ...
    | dir :: dirs ->
        ...
        if Path.exists fn then
          Some (sub_dir, fn, Meta.load ~name:(Some root_name) fn)
        else
          (* Alternative layout *)
          let fn =
            Path.relative dir ("META." ^ Lib_name.to_string root_name)
          in
          if Path.exists fn then
            Some (dir, fn, Meta.load fn ~name:(Some root_name))
          else loop dirs
  in
  match loop t.paths with
  | None -> Hashtbl.add t.packages root_name (Error Not_found)
...
let root_packages t =
  ...
  in
  Lib_name.Set.union pkgs (Lib_name.Set.of_list (Lib_name.Map.keys t.builtins))
...
let all_packages t =
  ...
  |> List.sort ~compare:(fun (a : Package.t) b ->
         Lib_name.compare a.name b.name )
