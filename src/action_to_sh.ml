...
let simplify act =
  let rec loop (act : Action.For_shell.t) acc =
    ...
    | Ignore (outputs, act) -> Redirect (block act, outputs, Dev_null) :: acc
    ...
    | Write_file (x, y) -> Redirect (echo y, Stdout, File x) :: acc
    ...
    | Merge_files_into (srcs, extras, target) ->
        Sh
          (Printf.sprintf "{ echo -ne %s; cat %s; } | sort -u > %s"
             ...
             (List.map srcs ~f:quote_for_shell |> String.concat ~sep:" ")
...
let rec pp = function
  ...
  | Chdir dir -> Pp.hovbox ~indent:2 [Pp.string "cd"; Pp.space; quote dir]
  | Setenv (k, v) -> Pp.concat [Pp.string k; Pp.string "="; quote v]
  | Sh s -> Pp.string s
