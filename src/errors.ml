...
let die fmt = kerrf fmt ~f:(fun s -> raise (Exn.Fatal_error s))

let exnf t fmt =
  Format.pp_open_box err_ppf 0;
  Format.pp_print_as err_ppf 7 "";
  (* "Error: " *)
...
let fail t fmt =
  Format.pp_print_as err_ppf 7 "";
  (* "Error: " *)
  kerrf fmt ~f:(fun s -> raise (Exn.Loc_error (t, s)))

let fail_lex lb fmt = fail (Loc.of_lexbuf lb) fmt

let fail_opt t fmt = match t with None -> die fmt | Some t -> fail t fmt

let file_line path n =
  Io.with_file_in ~binary:false path ~f:(fun ic ->
      for _ = 1 to n - 1 do
...
let file_lines path ~start ~stop =
  Io.with_file_in ~binary:true path ~f:(fun ic ->
...
let print ppf loc =
      ...
      if Path.exists path then (
        ...
        if stop_c <= String.length line then
          ...
          if num_lines <= max_lines_to_print_in_full then
            let lines =
              file_lines path ~start:start.pos_lnum ~stop:stop.pos_lnum
            in
            ...
            let last_shown_lines =
              file_lines path
                ~start:(stop.pos_lnum - context_lines)
                ~stop:stop.pos_lnum
            in
  ...
  Format.fprintf ppf "@{<loc>File \"%s\", line %d, characters %d-%d:@}@\n%a"
    start.pos_fname start.pos_lnum start_c stop_c pp_file_excerpt ()
...
let warn t fmt =
  kerrf ~f:print_to_console ("%a@{<warning>Warning@}: " ^^ fmt ^^ "@.") print t
