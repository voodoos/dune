...
let map_result : type a b.
    (a, b) failure_mode -> int Fiber.t -> f:(unit -> a) -> b Fiber.t =
 fun mode t ~f ->
  match mode with
  | Strict -> t >>| fun _ -> f ()
  | Accept _ -> ( t >>| function 0 -> Ok (f ()) | n -> Error n )
...
module Fancy = struct
  ...
  let command_line ~prog ~args ~dir ~stdout_to ~stderr_to =
    ...
    | ( (File fn1 | Opened_file {filename = fn1; _})
      , (File fn2 | Opened_file {filename = fn2; _}) )
      when Path.equal fn1 fn2 ->
  ...
  let pp_purpose ppf = function
    | Internal_job -> Format.fprintf ppf "(internal)"
    | Build_job targets ->
        let rec split_paths targets_acc ctxs_acc = function
          ...
          | path :: rest -> (
              let add_ctx ctx acc =
                if ctx = "default" then acc else ctx :: acc
              in
              match Utils.analyse_target path with
              | Other path ->
                  split_paths
                    (Path.to_string path :: targets_acc)
                    ctxs_acc rest
              ...
              | Alias (ctx, name) ->
                  split_paths
                    (("alias " ^ Path.to_string name) :: targets_acc)
                    (add_ctx ctx ctxs_acc) rest )
        in
        ...
        let pp_contexts ppf = function
          ...
          | ctxs ->
              Format.fprintf ppf " @{<details>[%a]@}"
                (Format.pp_print_list ~pp_sep:pp_comma (fun ppf s ->
                     Format.fprintf ppf "%s" s ))
                ctxs
        in
        Format.fprintf ppf "%a%a"
          (Format.pp_print_list ~pp_sep:pp_comma pp_group)
          target_names_grouped_by_prefix pp_contexts contexts
end
...
let run_internal ?dir ?(stdout_to = Terminal) ?(stderr_to = Terminal) ~env
  ...
  let command_line =
    Fancy.command_line ~prog ~args ~dir ~stdout_to ~stderr_to
  in
  ...
  let output_filename, stdout_fd, stderr_fd, to_close =
    ...
    | (Terminal, _ | _, Terminal) when !Clflags.capture_outputs ->
        ...
        let fd =
          Unix.openfile (Path.to_string fn) [O_WRONLY; O_SHARE_DELETE] 0
        in
        (Some fn, fd, fd, Some fd)
    | _ -> (None, Unix.stdout, Unix.stderr, None)
  in
  ...
  | WSIGNALED n ->
      if display = Verbose then
        ...
        die
          "@{<error>%12s@} %a @{<error>(got signal %s)@}\n@{<details>%s@}\n%s"
...
let run_capture_gen ?dir ?stderr_to ~env ?(purpose = Internal_job) fail_mode
  ...
  let fn = Temp.create "dune" ".output" in
  map_result fail_mode
    (run_internal ?dir ~stdout_to:(File fn) ?stderr_to ~env ~purpose fail_mode
       prog args) ~f:(fun () ->
      let x = f fn in
      Temp.destroy fn;
      x )
...
let run_capture_line ?dir ?stderr_to ~env ?(purpose = Internal_job) fail_mode
    prog args =
  run_capture_gen ?dir ?stderr_to ~env ~purpose fail_mode prog args
    ~f:(fun fn ->
