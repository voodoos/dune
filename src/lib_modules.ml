...
let make_alias_module ~dir ~lib ~main_module_name ~modules =
  ...
  let alias_prefix =
    String.uncapitalize (Module.Name.to_string main_module_name)
  in
  if implements then
    let alias_prefix =
      sprintf "%s__%s__" alias_prefix (Lib_name.Local.to_string (snd lib.name))
    in
    let name = Module.Name.of_string alias_prefix in
    Some
      (Module.make name ~visibility:Public
         ~impl:
  ...
  then None
    ...
    Some
      (Module.make
         (Module.Name.add_suffix main_module_name "__")
         ~visibility:Public
         ~impl:
           (Module.File.make OCaml
    ...
    Some
      (Module.make main_module_name ~visibility:Public
         ~impl:
           (Module.File.make OCaml
...
let wrap_modules ~modules ~lib ~main_module_name =
  ...
  let prefix =
    if not (Dune_file.Library.is_impl lib) then fun _ -> main_module_name
    else
      ...
      let private_module_prefix =
        ...
        else main_module_name
      in
      fun m ->
        if Module.is_private m then private_module_prefix else main_module_name
  in
  Module.Name.Map.map modules ~f:(fun (m : Module.t) ->
      if
        Module.name m = main_module_name
        || Dune_file.Library.special_compiler_module lib m
      then m
      else Module.with_wrapper m ~main_module_name:(prefix m) )
    ...
    if transition then
      ...
      , Module.Name.Map.remove modules main_module_name
        |> Module.Name.Map.filter_map ~f:(fun m ->
               if Module.is_public m then Some (Module.wrapped_compat m)
               else None ) )
    else (wrap_modules ~modules ~main_module_name ~lib, Module.Name.Map.empty)
  in
  let alias_module = make_alias_module ~main_module_name ~dir ~lib ~modules in
  { modules
  ; alias_module
...
let make (lib : Dune_file.Library.t) ~dir (modules : Module.Name_map.t)
  ...
  | (Yes_with_transition _ | Simple true), None -> assert false
...
let alias t =
  ...
  | Some alias_module, Some main_module_name ->
      Some {Alias_module.main_module_name; alias_module}
...
let installable_modules t =
  ...
  in
  match t.alias_module with None -> modules | Some alias -> alias :: modules

let lib_interface_module t =
  if t.implements then None
  else
...
let set_modules t pped_modules = {t with modules = pped_modules}
...
let public_modules t = Module.Name.Map.filter ~f:Module.is_public t.modules

let have_artifacts t =
  let base = Module.Name.Map.superpose t.modules t.wrapped_compat in
  match t.alias_module with
  | None -> base
