...
let invalid_module_name ~loc = of_sexp_errorf loc "invalid module name: %S"
...
let relative_file =
  plain_string (fun ~loc fn ->
      ...
      else of_sexp_errorf loc "relative filename expected" )

let c_name, cxx_name =
  let make what ext =
    plain_string (fun ~loc s ->
        if match s with "" | "." | ".." -> true | _ -> false then
          of_sexp_errorf loc "%S is not a valid %s name." s what what ext
        else (loc, s) )
  in
  (make "C" "c", make "C++" "cpp")
...
module Pkg = struct
  ...
  let decode =
    ...
    match resolve p name with Ok x -> x | Error e -> Errors.fail loc "%s" e
...
module Pps_and_flags = struct
  module Jbuild_syntax = struct
    let of_string ~loc s =
      if String.is_prefix s ~prefix:"-" then Right [s]
      else Left (loc, Lib_name.of_string_exn ~loc:(Some loc) s)

    let item =
      peek_exn
      >>= function
      | Template {loc; _} -> no_templates loc "in the preprocessors field"
      | Atom _ | Quoted_string _ -> plain_string of_string
      | List _ -> list string >>| fun l -> Right l

    let split l =
      let pps, flags = List.partition_map l ~f:(fun x -> x) in
  ...
  let decode =
    switch_file_kind ~jbuild:Jbuild_syntax.decode ~dune:Dune_syntax.decode
end
...
module Bindings = struct
  ...
  let jbuild elem = list (elem >>| fun x -> Unnamed x)
         ...
         | Left (loc, name, values) :: l ->
             let vars =
               if not (String.Set.mem vars name) then String.Set.add vars name
                 ...
                 of_sexp_errorf loc
                   "Variable %s is defined for the second time." name
             in
             loop vars (Named (name, values) :: acc) l
       in
       loop String.Set.empty [] l)

  let decode elem = switch_file_kind ~jbuild:(jbuild elem) ~dune:(dune elem)
  ...
  let to_sexp sexp_of_a bindings =
    Sexp.List
      (List.map bindings ~f:(function
        ...
        | Named (name, bindings) ->
            Sexp.List
              ( Sexp.Encoder.string (":" ^ name)
              :: List.map ~f:sexp_of_a bindings ) ))
end
...
module Dep_conf = struct
  ...
  let encode = function
    ...
    | Source_tree t ->
        List
          [ Dune_lang.unsafe_atom_of_string "files_recursively_in"
          ; String_with_vars.encode t ]
    | Package t ->
        List
...
module Preprocess = struct
  ...
  let decode =
    sum
      ...
      ; ( "action"
        , located Action.Unexpanded.decode >>| fun (loc, x) -> Action (loc, x)
        )
      ; ( "pps"
        , let%map loc = loc
  ...
  let decode ~default a =
    ...
    | List (loc, Atom (_, A "per_module") :: _) ->
        sum
          [ ( "per_module"
            , repeat
                (pair a (list module_name) >>| fun (pp, names) -> (names, pp))
              >>| fun x ->
              of_mapping x ~default
              |> function
              | Ok t -> t
              | Error (name, _, _) ->
                  of_sexp_errorf loc "module %s present in two different sets"
         ...
         until_keyword "->"
           ~before:
             (let%map s = string
              ...
              else Left (Lib_name.of_string_exn ~loc:(Some loc) s))
           ~after:file
       in
       match file with
       | None -> of_sexp_errorf loc "(<[!]libraries>... -> <file>) expected"
       | Some file ->
...
module Auto_format = struct
  ...
  let language = sum [("ocaml", return Ocaml); ("reason", return Reason)]
  ...
  let enabled_for_field =
    ...
    match r with Some l -> Only l | None -> Default

  let enabled_for_to_sexp = function
  ...
  let to_sexp {enabled_for; loc = _} =
    Sexp.List [List [Atom "enabled_for"; enabled_for_to_sexp enabled_for]]

  let dparse_args =
    let%map loc = loc
    and enabled_for = record enabled_for_field in
    ({loc; enabled_for}, [])

  let key = Dune_project.Extension.register syntax dparse_args to_sexp
end
...
module Buildable = struct
  ...
  let single_preprocess t =
    ...
    else Preprocess.No_preprocessing
end
    ...
    let syntax =
      let syntax =
        Syntax.create ~name:"in_development_do_not_use_variants"
          ~desc:"the experimental variants feature" [(0, 1)]
      in
  ...
  module Stdlib = struct
    ...
    let glob =
      plain_string (fun ~loc str ->
          ...
          | Ok re -> Re.compile re
          | Error (_pos, msg) -> Errors.fail loc "invalid glob: %s" msg )
  ...
  let decode =
    record
      (let%map buildable = Buildable.decode
       ...
       and sub_systems =
         return () >>= fun () -> Sub_system_info.record_parser ()
       ...
       |> Option.iter ~f:(fun (virtual_modules, (_, impl)) ->
              of_sexp_errorf
                (Ordered_set_lang.loc virtual_modules |> Option.value_exn)
                "A library cannot be both virtual and implement %s"
                (Lib_name.to_string impl) );
       ( match (virtual_modules, wrapped, implements) with
       ...
       let self_build_stubs_archive =
         ...
         | Some name ->
             of_sexp_errorf loc
               "A library cannot use (self_build_stubs_archive ...) and (%s \
                ...) simultaneously."
               name
       in
       { name
        ...
        | None, Some public_names ->
            if dune_syntax >= (1, 1) then
              List.map public_names ~f:(fun (loc, p) ->
                  ...
                  | None ->
                      of_sexp_error loc
                        "This executable must have a name field"
                  | Some s -> (loc, s) )
            else
              of_sexp_errorf loc
        ...
        | None, None ->
            if dune_syntax >= (1, 1) then
              ...
              of_sexp_errorf loc "field %s is missing"
                (pluralize ~multi "name")
      in
      let t = {names; link_flags; link_deps; modes; buildable} in
      let has_public_name =
      ...
      let to_install =
        ...
        | None when has_public_name ->
            let mode_to_string mode =
              " - " ^ Dune_lang.to_string ~syntax:Dune (Link_mode.encode mode)
            in
            let mode_strings =
              List.map ~f:mode_to_string Link_mode.installable_modes
            in
        ...
        | Some mode ->
            ...
            let public_names =
              ...
              | Some pns -> pns
            in
            List.map2 names public_names ~f:(fun (_, name) (_, pub) ->
                Option.map pub ~f:(fun pub ->
                    {Install_conf.src = name ^ ext; dst = Some pub} ) )
            |> List.filter_opt
      in
...
module Rule = struct
  module Targets = struct
    ...
    let decode_static =
      ...
      and targets = list String_with_vars.decode in
  ...
  let short_form =
    located Action.Unexpanded.decode
    >>| fun (loc, action) ->
    { targets = Infer
    ; deps = Bindings.empty
    ; action = (loc, action)
  ...
  let long_form =
    ...
    and enabled_if = enabled_if in
    {targets; deps; action; mode; locks; loc; enabled_if}

  let jbuild_syntax =
    peek_exn
    >>= function List (_, Atom _ :: _) -> short_form | _ -> record long_form
  ...
  let dune_syntax =
    ...
    | List (_, Atom (loc, A s) :: _) -> (
      ...
      | None ->
          of_sexp_errorf loc
            ~hint:{on = s; candidates = String.Map.keys atom_table}
  ...
  let decode = switch_file_kind ~jbuild:jbuild_syntax ~dune:dune_syntax
  ...
  let ocamllex_dune =
    if_eos
      ~then_:(return {modules = []; mode = Standard; enabled_if = Blang.true_})
      ~else_:
        (if_list
           ~then_:
             (record
  ...
  let ocamllex = switch_file_kind ~jbuild:ocamllex_jbuild ~dune:ocamllex_dune
  ...
  let decode =
    record
      (let%map merge_into = field_o "merge_into" string
       ...
       in
       {merge_into; flags; modules; mode; loc = Loc.none; infer; enabled_if})
      ...
      (let%map buildable = Buildable.decode
       ...
       and action =
         field_o "action"
           ...
           >>> Action.Unexpanded.decode )
       in
       { exes =
           {Executables.link_flags; link_deps = []; modes; buildable; names}
       ; locks
       ; package
       ; deps
       ; enabled_if
...
module Include_subdirs = struct
  ...
  let decode = enum [("no", No); ("unqualified", Unqualified)]
end
...
module Stanzas = struct
  ...
  let rec parse stanza_parser ~lexer ~current_file ~include_stack sexps =
    List.concat_map sexps
      ~f:(Dune_lang.Decoder.parse stanza_parser Univ_map.empty)
         ...
         | Include (loc, fn) ->
             ...
             if
               List.exists include_stack ~f:(fun (_, f) ->
                   Path.equal f current_file )
             then raise (Include_loop (current_file, include_stack));
          ...
          Errors.fail loc
            ...
            (String.concat ~sep:""
               (List.map rest ~f:(fun x ->
                    sprintf "\n--> included from %s" (line_loc x) )))
    in
    match
      List.filter_map stanzas ~f:(function Dune_env.T e -> Some e | _ -> None)
    with
    | _ :: e :: _ ->
