...
let eval =
  let module Value = struct
    ...
    let key = function Error s -> s | Ok m -> Module.name m
  end in
  ...
  let missing_modules =
    Module.Name.Map.fold intf_only ~init:[]
      ~f:(fun ((_, (module_ : Module.t)) as module_loc) acc ->
        if Module.has_impl module_ then module_loc :: acc else acc )
  in
  let errors =
    Module.Name.Map.fold virtual_modules ~init:Module_errors.empty
      ~f:(fun ((_, (module_ : Module.t)) as module_loc) acc ->
        if Module.has_impl module_ then
          ...
          { acc with
            private_virt_modules = module_loc :: acc.private_virt_modules }
        else acc )
  in
  let missing_intf_only =
    Module.Name.Map.fold modules ~init:[]
      ~f:(fun ((_, (module_ : Module.t)) as module_loc) acc ->
        if Module.has_impl module_ then acc
        else if
          (not (Module.Name.Map.mem intf_only (Module.name module_)))
...
let check_invalid_module_listing ~(buildable : Buildable.t) ~intf_only ~modules
    ~virtual_modules ~private_modules =
  let { Module_errors.missing_modules
      ; missing_intf_only
      ; virt_intf_overlaps
      ; private_virt_modules } =
  ...
  in
  let uncapitalized =
    List.map ~f:(fun (_, m) -> Module.name m |> Module.Name.uncapitalize)
  in
  let line_list modules =
    List.map
      ~f:(fun (_, m) ->
        Module.name m |> Module.Name.to_string |> sprintf "- %s" )
      modules
    |> String.concat ~sep:"\n"
  in
  ( match private_virt_modules with
  | [] -> ()
  | (loc, _) :: _ ->
      Errors.fail loc
  ...
  ( if missing_intf_only <> [] then
    ...
    | None ->
        Errors.warn buildable.loc
          ...
          (let tag =
             Dune_lang.unsafe_atom_of_string "modules_without_implementation"
           in
           let modules =
...
let eval ~modules:(all_modules : Module.Name_map.t)
    ~buildable:(conf : Buildable.t) ~virtual_modules ~private_modules =
  ...
  let fake_modules, intf_only =
    ...
    in
    (Module.Name.Map.superpose fake_modules' fake_modules, intf_only)
  in
  let fake_modules, virtual_modules =
    ...
    | Some virtual_modules ->
        ...
        in
        (Module.Name.Map.superpose fake_modules' fake_modules, virtual_modules)
  in
  let fake_modules, private_modules =
    let fake_modules', private_modules =
      eval ~standard:Module.Name.Map.empty ~all_modules private_modules
    in
    (Module.Name.Map.superpose fake_modules' fake_modules, private_modules)
  in
