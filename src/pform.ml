...
module Map = struct
  ...
  let create ~(context : Context.t) ~cxx_flags =
    ...
    let make =
      match Bin.make with None -> string "make" | Some p -> path p
    in
  ...
  let rec expand map ~syntax_version ~pform =
    ...
    String.Map.find map name
    >>= fun v ->
    let describe = String_with_vars.Var.describe in
    match v with
    | No_info v -> Some v
    | Since (v, min_version) ->
        if syntax_version >= min_version then Some v
        else
          Syntax.Error.since
    ...
    | Deleted_in (v, in_version, repl) ->
        if syntax_version < in_version then Some v
        else
  ...
  let of_bindings bindings =
    { vars =
        Dune_file.Bindings.fold bindings ~init:String.Map.empty
          ~f:(fun x acc ->
            match x with
            | Unnamed _ -> acc
