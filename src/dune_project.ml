...
module Kind = struct
  ...
  let to_sexp t =
    Sexp.Atom (match t with Dune -> "dune" | Jbuilder -> "jbuilder")
end
...
module Name : sig
  ...
  let to_sexp = function
    ...
    | Anonymous p -> List [Atom "anonymous"; Path.to_sexp p]
  ...
  let anonymous path =
    if Path.is_managed path then Some (Anonymous path) else None

  let decode =
    Dune_lang.Decoder.plain_string (fun ~loc s ->
        if validate s then Named s
        else Dune_lang.Decoder.of_sexp_errorf loc "invalid project name" )
  ...
  let to_encoded_string = function
    ...
    | Anonymous p ->
        if Path.is_root p then "."
          ...
          "."
          ^ String.map (Path.to_string p) ~f:(function '/' -> '.' | c -> c)
...
module Project_file = struct
  ...
  let to_sexp {file; exists} =
    Sexp.Encoder.(
      record [("file", Path.to_sexp file); ("exists", bool exists)])
end
...
module Project_file_edit = struct
  ...
  let notify_user s = kerrf ~f:print_to_console "@{<warning>Info@}: %s\n" s
...
module Extension = struct
  ...
  let register_simple ?experimental syntax stanzas =
    ...
    let _ : unit t = register ?experimental syntax unit_stanzas unit_to_sexp in
    ()
  ...
  let automatic ~project_file ~f =
    Hashtbl.foldi extensions ~init:[] ~f:(fun name extension acc ->
        if f name then
          let version =
            if is_experimental extension then (0, 0)
            else Syntax.greatest_supported_version (syntax extension)
          in
          let parse_args p =
...
let get_exn () =
  ...
  get key
  >>| function
  | Some t -> t | None -> Exn.code_error "Current project is unset" []

let filename = "dune-project"

let get_local_path p =
  match Path.kind p with External _ -> assert false | Local p -> p
  ...
  | Some (_, pkg) -> (
      let pkg =
        ...
        Package.Name.Map.fold packages ~init:pkg ~f:(fun pkg acc ->
            if acc.Package.name <= pkg.Package.name then acc else pkg )
      in
      ...
      | None ->
          Errors.fail
            (Loc.in_file (Path.to_string (Package.opam_file pkg)))
            "%S is not a valid opam package name." name )

let name_field ~dir ~packages =
  let%map name = field_o "name" Name.decode in
  match name with Some x -> x | None -> default_name ~dir ~packages
           ...
           List.fold_left extensions ~init:(Univ_map.empty, [])
             ~f:(fun (args_acc, stanzas_acc)
               ...
               let args =
                 ...
                 in
                 let new_args_acc =
                   if is_explicit then Univ_map.add args_acc e.key arg
                   else args_acc
                 in
  ...
  let packages =
    String.Set.fold files ~init:[] ~f:(fun fn acc ->
        ...
        | pkg, ".opam" when pkg <> "" ->
            ...
            (name, {Package.name; path = dir; version_from_opam_file}) :: acc
        | _ -> acc )
    |> Package.Name.Map.of_list_exn
  in
  if String.Set.mem files filename then Some (load_dune_project ~dir packages)
