...
let rules (t : Dune_file.Tests.t) ~sctx ~dir ~scope ~dir_contents ~dir_kind =
  let test_kind (loc, name) =
    ...
    if String.Set.mem files expected_basename then
      `Expect
        { Action.Unexpanded.Diff.file1 =
            String_with_vars.make_text loc expected_basename
  ...
  List.iter t.exes.names ~f:(fun (loc, s) ->
      ...
      let extra_bindings =
        ...
        let test_exe_path =
          Super_context.Action.map_exe sctx (Path.relative dir test_exe)
        in
      ...
      | `Expect diff ->
          let rule =
            ...
            ; action =
                ( loc
                , Action.Unexpanded.Redirect (Stdout, diff.file2, run_action)
                )
            ; mode = Standard
            ; locks = t.locks
            ; loc
          ...
          ignore
            ( Simple_rules.user_rule sctx rule ~extra_bindings ~dir ~scope
              : Path.t list ) );
