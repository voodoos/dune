...
struct
  ...
  let rec encode =
    ...
    | Copy (x, y) -> List [atom "copy"; path x; path y]
    | Symlink (x, y) -> List [atom "symlink"; path x; path y]
    | Copy_and_add_line_directive (x, y) -> List [atom "copy#"; path x; path y]
...
module Make_mapper (Src : Action_intf.Ast) (Dst : Action_intf.Ast) = struct
  let map_one_step f (t : Src.t) ~dir ~f_program ~f_string ~f_path : Dst.t =
    ...
    | Setenv (var, value, t) ->
        Setenv
          ( f_string ~dir var
          , f_string ~dir value
          , f t ~dir ~f_program ~f_string ~f_path )
    | Redirect (outputs, fn, t) ->
        Redirect
          (outputs, f_path ~dir fn, f t ~dir ~f_program ~f_string ~f_path)
    | Ignore (outputs, t) ->
        Ignore (outputs, f t ~dir ~f_program ~f_string ~f_path)
    | Progn l ->
        Progn (List.map l ~f:(fun t -> f t ~dir ~f_program ~f_string ~f_path))
    | Echo xs -> Echo (List.map xs ~f:(f_string ~dir))
    | Cat x -> Cat (f_path ~dir x)
    | Copy (x, y) -> Copy (f_path ~dir x, f_path ~dir y)
    | Symlink (x, y) -> Symlink (f_path ~dir x, f_path ~dir y)
    ...
    | Diff {optional; file1; file2; mode} ->
        Diff
          {optional; file1 = f_path ~dir file1; file2 = f_path ~dir file2; mode}
...
module For_shell = struct
  ...
  include Make_ast (String_with_sexp) (String_with_sexp) (String_with_sexp)
            (Ast)
end
...
let for_shell t =
  let rec loop t ~dir ~f_program ~f_string ~f_path =
    ...
    | t -> Relativise.map_one_step loop t ~dir ~f_program ~f_string ~f_path
  in
...
module Unresolved = struct
  module Program = struct
    ...
    let of_string ~dir ~loc s =
      if String.contains s '/' then This (Path.relative dir s)
      else Search (loc, s)
  end
...
module Unexpanded = struct
  ...
  include Make_ast (String_with_vars) (String_with_vars) (String_with_vars)
            (Uast)
  ...
  let check_mkdir loc path =
    if not (Path.is_managed path) then
      Errors.fail loc
        ...
        (Dune_lang.pp Dune)
        (List
           [Dune_lang.unsafe_atom_of_string "mkdir"; Path_dune_lang.encode path])
  ...
  module Partial = struct
    ...
    module E = struct
      let expand ~ectx:{dir; _} ~mode ~f ~l ~r =
        Either.map ~l ~r:(fun s ->
            r
              ...
              (String_with_vars.expand s ~dir ~f ~mode)
              ~dir )

      let string =
        expand ~mode:Single ~l:(fun x -> x) ~r:(ignore_loc Value.to_string)

      let strings =
        expand ~mode:Many ~l:(fun x -> [x]) ~r:(ignore_loc Value.L.to_strings)
      ...
      let path e =
        ...
        expand ~mode:Single
          ~l:(fun x -> x)
          ~r:(ignore_loc Value.(to_path ?error_loc))
          e
    ...
    let rec expand t ~ectx ~map_exe ~f : Unresolved.t =
      ...
      | Ignore (outputs, t) -> Ignore (outputs, expand t ~ectx ~map_exe ~f)
      ...
      | Copy (x, y) -> Copy (E.path ~ectx ~f x, E.path ~ectx ~f y)
      | Symlink (x, y) -> Symlink (E.path ~ectx ~f x, E.path ~ectx ~f y)
      | Copy_and_add_line_directive (x, y) ->
      ...
      | Rename (x, y) -> Rename (E.path ~ectx ~f x, E.path ~ectx ~f y)
      | Remove_tree x -> Remove_tree (E.path ~ectx ~f x)
      | Mkdir x -> (
        match x with
        ...
        | Right tmpl ->
            ...
            Mkdir path )
      | Digest_files x -> Digest_files (List.map x ~f:(E.path ~ectx ~f))
      | Diff {optional; file1; file2; mode} ->
          Diff
            { optional
  ...
  let rec partial_expand t ~ectx ~map_exe ~f : Partial.t =
    ...
    | Setenv (var, value, t) ->
        Setenv
          ( E.string ~ectx ~f var
          , E.string ~ectx ~f value
          , partial_expand t ~ectx ~map_exe ~f )
    | Redirect (outputs, fn, t) ->
        Redirect
          (outputs, E.path ~ectx ~f fn, partial_expand t ~ectx ~map_exe ~f)
    | Ignore (outputs, t) ->
...
let fold_one_step t ~init:acc ~f =
  match t with
  | Chdir (_, t) | Setenv (_, _, t) | Redirect (_, _, t) | Ignore (_, t) ->
      f acc t
  | Progn l -> List.fold_left l ~init:acc ~f
  | Run _ | Echo _ | Cat _ | Copy _ | Symlink _
   |Copy_and_add_line_directive _ | System _ | Bash _ | Write_file _
   |Rename _ | Remove_tree _ | Mkdir _ | Digest_files _ | Diff _
   |Merge_files_into _ ->
      acc
...
module Infer = struct
  ...
  struct
    ...
    let rec infer acc t =
      ...
      | Copy (src, dst)
       ...
       |Symlink (src, dst) ->
          acc +< src +@ dst
      | Chdir (_, t) | Setenv (_, _, t) | Ignore (_, t) -> infer acc t
      ...
      | Echo _ | System _ | Bash _ | Remove_tree _ | Mkdir _ -> acc
  ...
  include Make (Ast) (Path.Set) (Outcome)
            (struct
              let ( +@ ) acc fn =
                {acc with targets = Path.Set.add acc.targets fn}
  ...
  module Partial =
    Make (Unexpanded.Partial.Past) (Path.Set) (Outcome)
      (struct
        let ( +@ ) acc fn =
          match fn with
  ...
  let partial ~all_targets t =
    if all_targets then Partial_with_all_targets.infer t else Partial.infer t
  ...
  module Unexp =
    Make (Unexpanded.Uast) (S_unexp) (Outcome_unexp)
      (struct
        open Outcome_unexp

        let ( +@ ) acc fn =
          if String_with_vars.is_var fn ~name:"null" then acc
          else {acc with targets = fn :: acc.targets}
...
let symlink_managed_paths sandboxed deps =
  let steps =
    Path.Set.fold (Deps.paths deps) ~init:[] ~f:(fun path acc ->
        if Path.is_managed path then Symlink (path, sandboxed path) :: acc
        else acc )
  in
  Progn steps
  ...
  Progn
    ...
    ; Progn
        (List.filter_map targets ~f:(fun path ->
             if Path.is_managed path then Some (Rename (sandboxed path, path))
             else None )) ]
