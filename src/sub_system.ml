...
module Register_backend (M : Backend) = struct
  ...
  module Set = Set.Make (struct
    ...
    let compare a b =
      compare (Lib.unique_id (M.lib a)) (Lib.unique_id (M.lib b))
  end)
  ...
  let resolve db (loc, name) =
    ...
    | None ->
        Error
          (Errors.exnf loc "%a is not %s %s" Lib_name.pp_quoted name
  ...
  module Selection_error = struct
    ...
    let to_exn t ~loc =
      ...
      | Too_many_backends backends ->
          Errors.exnf loc "Too many independent %s found:\n%s"
            (M.desc ~plural:true)
            (String.concat ~sep:"\n"
...
module Register_end_point (M : End_point) = struct
  ...
  let gen info (c : Library_compilation_context.t) =
    let open Result.O in
    let backends =
      Lib.Compile.direct_requires c.compile_info
      >>= fun deps ->
      Lib.Compile.pps c.compile_info
      >>= fun pps ->
