...
(** Create a new build system. [file_tree] represent the source tree. *)
val create :
...
(** Set the rule generators callback. There must be one callback per build
    ...
    corresponding build context. It receive the directory for which to generate
    the rules and the split part of the path after the build context. It must
    return an additional list of sub-directories to keep. This is in addition
    to the ones that are present in the source tree and the ones that already
    contain rules.
...
(** Add a rule to the system. This function must be called from the [gen_rules]
    ...
    Assuming that [gen_rules ~dir:a] calls [add_rule r] where [r.dir] is [Some
    ...
    dependency from [b] to [a]. There must be no cyclic directory dependencies. *)
...
(** [eval_glob t ~dir re ~f] returns the list of files in [dir] that matches
    [re] to [f]. The list of files includes the list of targets. *)
...
(** [on_load_dir ~dir ~f] remembers to run [f] when loading the rules for
    [dir]. *)
...
(** Return all the library dependencies required to build this request, by
    context name *)
