...
let dev_mode_warnings =
  "@a"
  ^ String.concat ~sep:""
      (List.map ~f:(sprintf "-%d") [4; 29; 40; 41; 42; 44; 45; 48; 58; 59; 60])
...
let default_flags ~profile =
  if profile = "dev" then
    [ "-w"
    ; dev_mode_warnings ^ default_warnings
    ; "-strict-sequence"
    ; "-strict-formats"
    ; "-short-paths"
    ; "-keep-locs" ]
...
let make ~flags ~ocamlc_flags ~ocamlopt_flags ~default ~eval =
  let f name x standard =
    Build.memoize name
      ( if Ordered_set_lang.Unexpanded.has_special_forms x then eval x ~standard
      else eval x ~standard:(Build.return []) )
  in
