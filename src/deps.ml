...
let trace_path fn = (Path.to_string fn, Utils.Cached_digest.file fn)
...
let path_union a b = Path.Set.union a.paths b.paths

let path_diff a b = Path.Set.diff a.paths b.paths

let empty = {paths = Path.Set.empty; vars = String.Set.empty}

let add_path t path = {t with paths = Path.Set.add t.paths path}

let add_paths t fns = {t with paths = Path.Set.union t.paths fns}

let add_env_var t var = {t with vars = String.Set.add t.vars var}
...
let to_sexp {paths; vars} =
  ...
  in
  Dune_lang.Encoder.record [("paths", sexp_paths); ("vars", sexp_vars)]
