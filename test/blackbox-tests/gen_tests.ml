...
module Test = struct
  ...
  let make ?env ?skip_ocaml ?(skip_platforms = []) ?(enabled = true)
      ?(js = false) ?(external_deps = false) name =
    {name; env; skip_ocaml; skip_platforms; external_deps; enabled; js}
  ...
  let pp_sexp fmt t =
    ...
    let action =
      match t.env with
      | None -> action
      | Some (k, v) -> List [atom "setenv"; atom_or_quoted_string k; v; action]
    in
