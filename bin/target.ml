...
let request (setup : Dune.Main.setup) targets =
  ...
  List.fold_left targets ~init:(Build.return ()) ~f:(fun acc target ->
      ...
      | Alias {Alias.name; recursive; dir; contexts} ->
          ...
          ( if recursive then Build_system.Alias.dep_rec_multi_contexts
          else Build_system.Alias.dep_multi_contexts )
            ~dir ~name ~file_tree:setup.file_tree ~contexts )

let log_targets ~log targets =
  List.iter targets ~f:(function
    | File path -> Log.info log @@ "- " ^ Path.to_string path
...
let target_hint (setup : Dune.Main.setup) path =
  ...
  let candidates =
    if Path.is_in_build_dir path then candidates
    else
...
let resolve_path path ~(setup : Dune.Main.setup) =
  ...
  let can't_build path = Error (path, target_hint setup path) in
  ...
  else if not (Path.is_managed path) then Ok [File path]
  else if Path.is_in_build_dir path then
    if Build_system.is_target setup.build_system path then Ok [File path]
    else can't_build path
  else
    match
  ...
  | _ ->
      let targets =
        List.map user_targets ~f:(function
          ...
          | Path p -> resolve_path p ~setup )
      in
      if common.config.display = Verbose then (
