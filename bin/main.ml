...
let restore_cwd_and_execve (common : Common.t) prog argv env =
  let prog =
    if Filename.is_relative prog then Filename.concat common.root prog
    else prog
  in
...
module Main = struct
  ...
  let setup ~log ?external_lib_deps_mode (common : Common.t) =
    setup ~log
      ...
      ?only_packages:common.only_packages ?external_lib_deps_mode ?x:common.x
...
module Log = struct
  ...
  let create (common : Common.t) = Log.create ~display:common.config.display ()
end
...
module Scheduler = struct
  ...
  let go ?log ~(common : Common.t) fiber =
    let fiber = Main.set_concurrency ?log common.config >>= fun () -> fiber in
    Scheduler.go ?log ~config:common.config fiber

  let poll ?log ~(common : Common.t) ~once ~finally () =
    let once () =
      Main.set_concurrency ?log common.config >>= fun () -> once ()
    in
  ...
  let term =
    ...
    Scheduler.go ~log:(Log.create common) ~common
      ...
      if na then (
        ...
        let max_len =
          String.longest_map pkgs ~f:(fun n ->
              Findlib.Package.name n |> Lib_name.to_string )
        in
...
let run_build_command ~log ~common ~targets =
  let once () =
    Main.setup ~log common >>= fun setup -> do_build setup (targets setup)
  in
  let finally () =
  ...
  in
  if common.watch then Scheduler.poll ~log ~common ~once ~finally ~canceled ()
...
let build_targets =
  ...
  let default_target =
    match Which_program.t with Dune -> "@@default" | Jbuilder -> "@install"
  in
  let term =
    let%map common = Common.term
    and targets = Arg.(value & pos_all string [default_target] name_) in
    Common.set_common common ~targets;
...
let runtest =
  ...
  let term =
    ...
    Common.set_common common
      ~targets:
        (List.map dirs ~f:(function
          | "" | "." -> "@runtest"
          | dir when dir.[String.length dir - 1] = '/' ->
              sprintf "@%sruntest" dir
        ...
        String.Map.foldi ~init:false
          ...
          ~f:(fun context_name lib_deps acc ->
            ...
            if only_missing then
              ...
              in
              if Lib_name.Map.is_empty missing then acc
              else if
                Lib_name.Map.for_alli missing ~f:(fun _ kind ->
                    kind = Lib_deps_info.Kind.Optional )
              then (
                Format.eprintf
                  "@{<error>Error@}: The following libraries are missing in \
                   the %s context:\n\
                   %s@."
                  context_name
...
let rules =
  ...
  let term =
    ...
    and makefile_syntax =
      Arg.(
        value & flag
        & info ["m"; "makefile"] ~doc:"Output the rules in Makefile syntax.")
    and targets =
      ...
      let print oc =
        ...
        if makefile_syntax then
          List.iter rules ~f:(fun (rule : Build_system.Rule.t) ->
              ...
              Format.fprintf ppf
                "@[<hov \
                 2>@{<makefile-stuff>%a:%t@}@]@,@<0>\t@{<makefile-action>%a@}@,@,"
                ...
                Pp.pp (Action_to_sh.pp action) )
        else
          List.iter rules ~f:(fun (rule : Build_system.Rule.t) ->
              let sexp =
                let paths ps =
                  Dune_lang.Encoder.list Path_dune_lang.encode
                    (Path.Set.to_list ps)
                in
...
let get_dirs context ~prefix_from_command_line ~libdir_from_command_line =
  ...
  | None ->
      Context.install_prefix context
      >>= fun prefix ->
      let libdir =
      ...
      in
      libdir >>| fun libdir -> (prefix, libdir)

let print_unix_error f =
  try f () with Unix.Unix_error (e, _, _) ->
    Format.eprintf "@{<error>Error@}: %s@." (Unix.error_message e)
...
module File_ops_real : FILE_OPERATIONS = struct
  let copy_file ~src ~dst ~executable =
    let chmod =
      if executable then set_executable_bits else clear_executable_bits
    in
...
let file_operations ~dry_run : (module FILE_OPERATIONS) =
  if dry_run then (module File_ops_dry_run) else (module File_ops_real)
...
let install_uninstall ~what =
  ...
  let term =
    ...
    and pkgs = Arg.(value & pos_all package_name [] name_) in
    Common.set_common common ~targets:[];
    let log = Log.create common in
    Scheduler.go ~log ~common
      ...
      >>= fun setup ->
      let pkgs =
        match pkgs with [] -> Package.Name.Map.keys setup.packages | l -> l
      in
      ...
      if missing_install_files <> [] then
        die
          ...
          (String.concat ~sep:"\n"
             (List.map missing_install_files ~f:(fun p ->
  ...
  let term =
    ...
    let prog_where =
      ...
      | Relative_to_current_dir ->
          ...
          `This_rel (Path.relative context.build_dir prog)
    in
    let targets =
      lazy
    ...
    let real_prog =
      ...
      | `Search prog ->
          let path =
            Config.local_install_bin_dir ~context:context.name :: context.path
          in
          Bin.which prog ~path
    ...
    | None, true -> (
      ...
      | _ :: _ ->
          Format.eprintf
            "@{<Error>Error@}: Program %S isn't built yet you need to build \
             it first or remove the --no-build option.@."
            prog;
          raise Already_reported )
    | None, false ->
        Format.eprintf "@{<Error>Error@}: Program %S not found!@." prog;
...
let subst =
  let doc = "Substitute watermarks in source files." in
  let man =
    let var name desc =
  ...
  let term =
    ...
    | Dune ->
        ...
        let config : Config.t = {display = Quiet; concurrency = Fixed 1} in
    ...
    let build_system, context, utop_path =
      ...
      >>= (fun setup ->
            ...
            let target =
              ...
              | Ok _ -> assert false
            in
            do_build setup [File target]
            >>| fun () -> (setup.build_system, context, Path.to_string target)
            )
      |> Scheduler.go ~log ~common
    in
      ...
      | _ ->
          let files =
            List.map files ~f:(fun fn ->
                Path.of_string (Common.prefix_target common fn) )
          in
          let on_missing fn =
    ...
    Scheduler.go ~log ~common
      ...
      let pp ppf =
        Format.fprintf ppf "@[<v1>(@,@[<v>%a@]@]@,)"
          (Format.pp_print_list (Dune_lang.pp Dune))
      in
      match l with
...
let fmt =
  ...
  let man =
    [ `S "DESCRIPTION"
    ; `P
        {|$(b,dune unstable-fmt) reads a dune file and outputs a formatted
    ...
    if true then
      let input, output =
        ...
        | Some path, true ->
            ...
            (Some path, Some path)
        | Some path, false -> (Some (Arg.Path.path path), None)
        | None, true -> die "--inplace requires a file name"
      in
      Dune_fmt.format_file ~input ~output
    else
      die
        "This command is unstable. Please pass --unstable to use it \
         nonetheless."
  in
  ...
  let help =
    ...
    let term =
      ...
      @@ let%map man_format = Arg.man_format
         and what =
           Arg.(
             value & pos 0 (some (enum commands)) None & info [] ~docv:"TOPIC")
         in
         match what with
