...
let find_root () =
  ...
  let rec loop counter ~candidates ~to_cwd dir =
    ...
    if String.Set.mem files Workspace.filename then
      ...
      Which_program.t = Jbuilder
      && String.Set.exists files ~f:(fun fn ->
             String.is_prefix fn ~prefix:"jbuild-workspace" )
    then cont counter ~candidates:((1, dir, to_cwd) :: candidates) dir ~to_cwd
  ...
  and cont counter ~candidates ~to_cwd dir =
    if counter > String.length cwd then candidates
    else
      let parent = Filename.dirname dir in
      if parent = dir then candidates
      else
  ...
  | l ->
      let lowest_priority =
        List.fold_left l ~init:max_int ~f:(fun acc (prio, _, _) -> min acc prio)
      in
      let _, dir, to_cwd =
