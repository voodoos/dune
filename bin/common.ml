...
type t =
  ...
  ; config : Config.t
  ; default_target : string (* For build & runtest only *)
  ; watch : bool }

let prefix_target common s = common.target_prefix ^ s

let set_dirs c =
  if c.root <> Filename.current_dir_name then Sys.chdir c.root;
...
let set_common_other c ~targets =
  ...
  Clflags.external_lib_deps_hint :=
    List.concat
      [["dune"; "external-lib-deps"; "--missing"]; c.orig_args; targets]
...
let footer =
  `Blocks
    [`S "BUGS"; `P "Check bug reports at https://github.com/ocaml/dune/issues"]
...
let term =
  let incompatible a b =
    `Error (true, sprintf "Cannot use %s and %s simultaneously" a b)
  in
  ...
  let dump_opt name value =
    match value with None -> [] | Some s -> [name; s]
  in
  ...
  let%map concurrency =
    let arg =
      Arg.conv
        ( (fun s ->
            Result.map_error (Config.Concurrency.of_string s) ~f:(fun s ->
                `Msg s ) )
        , fun pp x ->
  ...
  and no_buffer =
    Arg.(
      ...
      & info ["no-buffer"] ~docs ~docv:"DIR"
          ~doc:
            {|Do not buffer the output of commands executed by dune.
    ...
    @@ let%map root =
       ...
       and config_file_opt, config_file =
         ...
         @@ let%map config_file =
            ...
            | Some fn, false ->
                `Ok (Some "--config-file", This (Arg.Path.path fn))
       ...
       and profile =
         ...
         @@ let%map dev =
              ...
              @@ let%map dev =
                   Arg.(
                     value & flag
                     & info ["dev"] ~docs ~doc:{|Same as $(b,--profile dev)|})
                 in
                 ...
                 | true, Dune ->
                     `Error
                       ( true
                       , "--dev is no longer accepted as it is now the default."
                       )
            and profile =
              Arg.(
                value
            ...
            | true, Some _ ->
                `Error (true, "Cannot use --dev and --profile simultaneously")
       and default_target =
         Arg.(
           value
           & opt (some string) None
       ...
       | None, _, _, _, _, _, _ ->
           `Ok
             ...
             , List.concat
                 ...
                 ; ( if ignore_promoted_rules then ["--ignore-promoted-rules"]
                   else [] )
  ...
  let root, to_cwd =
    match root with
    | Some dn -> (dn, [])
    | None -> if Config.inside_dune then (".", []) else Util.find_root ()
  in
  let orig_args =
    List.concat
      [dump_opt "--workspace" (Option.map ~f:Arg.Path.arg workspace_file); orig]
  in
  let config =
    match config_file with
    ...
    | Default ->
        if Config.inside_dune then Config.default
        else Config.load_user_config_file ()
  in
  let config = Config.merge config {display; concurrency} in
  let config =
